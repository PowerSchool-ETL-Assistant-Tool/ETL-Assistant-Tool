var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "ELPA_ALT";
    ...
    var scoresToMap = [
        {TEST: "ELPA_ALT", CODE:"TOT", FIELD: "ALT_ELPA_OVERALL", MAP_FUNCTION: mapScaleScore}
        ,{TEST: "ELPA_ALT", CODE:"COMP", FIELD: "ALT_ELPA_COMPREHENSION", MAP_FUNCTION: mapScaleScore}
        ,{TEST: "ELPA_ALT", CODE:"SPEAK", FIELD: "1A_DOMAIN_SPEAKING", MAP_FUNCTION: mapScaleScore}
        ,{TEST: "ELPA_ALT", CODE:"WRIT", FIELD: "1B_DOMAIN_WRITING", MAP_FUNCTION: mapScaleScore}
        ,{TEST: "ELPA_ALT", CODE:"LIST", FIELD: "2A_DOMAIN_LISTENING", MAP_FUNCTION: mapScaleScore}
        ,{TEST: "ELPA_ALT", CODE:"READ", FIELD: "2B_DOMAIN_READING", MAP_FUNCTION: mapScaleScore}
        ,{TEST: "ELPA_ALT", CODE:"SPEAK_WRIT", FIELD: "1_MODALITY_PRODUCTIVE_SPEAKING_AND_WRITING", MAP_FUNCTION: mapScaleScore}
        ,{TEST: "ELPA_ALT", CODE:"LIST_READ", FIELD: "2_MODALITY_RECEPTIVE_LISTENING_AND_READING", MAP_FUNCTION: mapScaleScore}
    ];
            ...
            switch(field) {
                ...
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = _util.coalesce(
                        ...
                        ,row.getField("DISTRICT_LOCAL_STUDENT_ID_NUMBER")
                    );
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("SCHOOL_VENDOR_ID"), row.getField("SCHOOL_LOCAL_ID"), row.getField("SCHOOL_STATE_ID"), row.getField("SCHOOL_NAME"), row.getField("ENROLLED_SCHOOL"));
                    break;
                case "SCHOOL_LOCAL_ID":
                     record.SCHOOL_LOCAL_ID = _util.coalesce(row.getField("SCHOOL_NUMBER"), row.getField("SCHOOL_CODE"), row.getField("ENROLLED_SCHOOL"));
                     break;
                case "SCHOOL_STATE_ID":
                     record.SCHOOL_STATE_ID = _util.coalesce(row.getField("SCHOOL_NUMBER"), row.getField("SCHOOL_CODE"), row.getField("ENROLLED_SCHOOL"));
                     break;
...
//This function takes the period from TEST_SEASON column of data file while removing any numeric characters
    ...
    function mapScaleScore(row, testMetadata, signature) {
        ...
        // Handle performance field mapping based on test type
        if (testMetadata.CODE === "TOT") {
            // For overall score, use proficiency determination
            performance = row.getField("ALT_ELPA_PROFICIENCY_DETERMINATION");
        } else {
            // For domain-specific scores, use performance field
            performance = row.getField(testMetadata.FIELD + "_PERFORMANCE");
        }
