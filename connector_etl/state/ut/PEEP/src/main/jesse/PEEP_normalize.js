var AppNormalize = (function () {
    ...
    var _assessmentIdentifier = "PEEP";
    ...
    var scoresToMap = [
        { TEST: "PEEP", CODE: "LIT", FIELD: "SUBJECT_AREA", MAP_FUNCTION: mapScaledScore }
    ];
    ...
    var exitPerfLevels = {
        ...
        "": { LEVEL: "", DESC: "", PASS_IND: "No" },
        null: { LEVEL: "", DESC: "", PASS_IND: "No" }
    };

    var entryPerfLevels = {
        "2": { LEVEL: "2", DESC: "At Benchmark", PASS_IND: "Yes" },
        "1": { LEVEL: "1", DESC: "Support Needed", PASS_IND: "No" },
        "": { LEVEL: "", DESC: "", PASS_IND: "No" },
        null: { LEVEL: "", DESC: "", PASS_IND: "No" }
    };
    /***********************************************************************************
...
//                    record.REPORTING_PERIOD = hierarchy.REPORTING_PERIOD;
    ...
    function mapScaledScore(row, testMetadata) {
        ...
        if (row.getField("SUBJECT_AREA") === "Literacy") {
            testNumber = testMetadata.TEST + "_LIT";
        }
        else if (row.getField("SUBJECT_AREA") === "Numeracy") {
            testNumber = testMetadata.TEST + "_NUM";
        }
        ...
        var rawScore = row.getField("TOTAL");
        ...
        if(fileName.indexOf('ENTRY') > -1){
            var decode = entryPerfLevels[perfLevel]
        } else {
            var decode = exitPerfLevels[perfLevel]
        }
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord, period) {
        ...
        if (srcRecord.getField("SCHOOL_NAME")) {
            assessmentAdminKey = assessmentAdminKey + "_"  + srcRecord.getField("SCHOOL_NAME").split(' ').join('_');
        }
