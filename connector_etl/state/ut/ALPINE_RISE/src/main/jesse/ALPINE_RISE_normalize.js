var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "ALPINE_RISE";
     ...
     ***************************************************************************/
     ...
     var scoresToMap = [
          {TEST: "ALPINE_RISE", CODE:"", FIELD: "", MAP_FUNCTION: mapOverallScore}
      ];
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "STUDENT_GENDER_CODE":
                    record.STUDENT_GENDER_CODE = "";
                    break;
                case "STUDENT_GRADE_CODE":
                    record.STUDENT_GRADE_CODE = gradeDecode[row.getField('STUDENT_GRADE_LEVEL')];
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("STATE_SCHOOL_ID"), row.getField("SCHOOL_NAME"));
                    break;
    ...
    function mapAdditionalAdminFields(row) {
        ...
        admin.TEST_EXEMPTION_CODE = row.getField("EXEMPT_CODE");
        admin.TEST_EXEMPTION_DESC = row.getField("EXEMPT_DESCRIP");
    ...
    function mapOverallScore(row, testMetadata) {
        var score = {};
        var testNumber = row.getField("HOONUIT_TEST_NUMBER");
        var scaleScore = row.getField("SCALE_SCORE");
        var proficiencyLvl = row.getField("PROFICIENCY_LEVEL");
        var proficiencyDes = row.getField("PROFICIENCY_DESCRIP");
        var growthPercentile = row.getField("SGP");
        var isProficient = row.getField("IS_PROFICIENT");
        var zScore = row.getField("ZSCORE");
        var particpationCode = row.getField("PARTICIPATION_CODE");
        var subTestName = row.getField("SUBTEST_NAME");


        // Check for key fields and do not map score and exit if conditions met.
        if((scaleScore === null || scaleScore === undefined || _util.trim(scaleScore) === "--" || _util.trim(scaleScore) === "") &&
           (growthPercentile === null || growthPercentile === undefined || _util.trim(growthPercentile) === "--" || _util.trim(growthPercentile) === "") &&
           (zScore === null || zScore === undefined || _util.trim(zScore) === "--" || _util.trim(zScore) === ""))
        {
            return null;
        }
    	...
    	score["TEST_PASSED_INDICATOR"] = passDecode[isProficient];
    	score["TEST_PRIMARY_RESULT_CODE"] = proficiencyLvl;
    	score["TEST_PRIMARY_RESULT"] = proficiencyDes;

        if(!isNaN(growthPercentile)){
    	score["TEST_GROWTH_PERCENTILE"] = growthPercentile;
    	}
        score["TEST_Z_SCORE"] = zScore;
        score["TEST_SCORE_ATTRIBUTES"] = subTestName;

        if(!isNaN(particpationCode)){
        score["TEST_CUSTOM_RESULT_CODE"] = particpationCode;
        }
