var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "NJ_PRE_POST";
...
//    }
    ...
    function mapNormalizedRecords(signature, hierarchy, rows) {
        ...
        if(signature.format.formatName === "NJ_LOCAL_GENERIC_99_COLS"){
            pType = "PRE";
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row, pType) {
        ...
        var finalDate = null;
                ...
                if(pType !== null && pType !== undefined && pType!== ""){
                    ...
                    var dateCompletedPrePost = _util.coalesce(row.getField(pType+"_TEST_ADMIN_DATE_STR"),row.getField(pType +"_DATESTUDENTCOMPLETEDTEST"));
                    if(dateCompletedPrePost!==null && dateCompletedPrePost !== 'NULL') {
                        if (dateCompletedPrePost.toString().indexOf("-") > -1) {
                            finalDate = AppUtil.getDateObjByPattern(dateCompletedPrePost, "dd-MM-yyyy").STANDARD_DATE;
                        }
                        else{
                            finalDate = AppUtil.getDateObjByPattern(dateCompletedPrePost, "MM/dd/yyyy").STANDARD_DATE;
                        }
                    }
                }else{
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "REPORTING_PERIOD":
                    record.REPORTING_PERIOD = _util.coalesce(pType,adminPeriodDecodes[row.getField("TEST_ADMIN_PERIOD")],hierarchy.REPORTING_PERIOD);
                    break;
                ...
                case "STUDENT_LOCAL_ID":
                    if(row.getField(pType + "_DISTRICT_STUDENT_ID") !== 'NULL'){
                        record.STUDENT_LOCAL_ID = _util.coalesce(row.getField("DISTRICT_STUDENT_ID")
                        , row.getField(pType + "_DISTRICT_STUDENT_ID"));
                    }
                    break;
                case "STUDENT_STATE_ID":
                    if(row.getField(pType + "_STATE_STUDENT_ID") !== 'NULL'){
                        record.STUDENT_STATE_ID = _util.coalesce(
                            row.getField("STATE_STUDENT_ID")
                            , row.getField("STATE_STUDENT_ID_VERSION_2")
                            , row.getField(pType + "STATE_STUDENT_ID")
                        );
                    }
                    break;
                case "STUDENT_VENDOR_ID":
                    if(row.getField(pType + "_DISTRICT_STUDENT_ID") !== 'NULL' || row.getField(pType + "_STATE_STUDENT_ID") !== 'NULL'){
                        record.STUDENT_VENDOR_ID = _util.coalesce(
                            row.getField("DISTRICT_STUDENT_ID")
                            , row.getField("STATE_STUDENT_ID")
                            ,row.getField("STUDENT_ID")
                            , row.getField(pType + "_DISTRICT_STUDENT_ID")
                            , row.getField(pType + "_STATE_STUDENT_ID")
                        );
                    }
                    break;
                case "STUDENT_FIRST_NAME":
                    record.STUDENT_FIRST_NAME = _util.coalesce(row.getField('STUDENT_FIRST_NAME'), row.getField(pType +  '_STUDENT_FIRST_NAME'));
                    break;
                case "STUDENT_LAST_NAME":
                    record.STUDENT_LAST_NAME = _util.coalesce(row.getField('STUDENT_LAST_NAME'), row.getField(pType +  '_STUDENT_LAST_NAME'));
                    break;
                case "STUDENT_MIDDLE_NAME":
                    record.STUDENT_MIDDLE_NAME= row.getField('STUDENT_MIDDLE_NAME');
                    break;
                case "STUDENT_GENDER_CODE":
                    record.STUDENT_GENDER_CODE = _util.coalesce(_util.coalesce(row.getField("GENDER"),row.getField(pType + "_GENDER"), "").substring(0,1));
                    break;
                case "STUDENT_GRADE_CODE":
                    record.STUDENT_GRADE_CODE = _util.coalesce(gradeDecode[row.getField("TEST_STUDENT_GRADE")], gradeDecode[row.getField(pType + "_TEST_STUDENT_GRADE")]);
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                        if(pType + "_VENDOR_SCHOOL_ID" !== 'NULL'){
                            ...
                            , row.getField(pType + "_VENDOR_SCHOOL_ID")
                        );
                        }
                    break;
                case "SCHOOL_LOCAL_ID":
                    if(pType + "_DISTRICT_SCHOOL_ID" !== 'NULL')
                     record.SCHOOL_LOCAL_ID = _util.coalesce(row.getField("DISTRICT_SCHOOL_ID")
                     , row.getField(pType + "_DISTRICT_SCHOOL_ID"));
                     break;
                case "SCHOOL_STATE_ID":
                    if(pType + "_SCHOOL_NUMBER" !== 'NULL'){
                        record.SCHOOL_STATE_ID = _util.coalesce(row.getField("SCHOOL_NUMBER"), row.getField(pType + "_SCHOOL_NUMBER"));
                    }
                     break;
                case "SCHOOL_NAME" :
                    if(pType + "_VENDOR_SCHOOL_ID" !== 'NULL') {
                        record.SCHOOL_NAME = _util.coalesce(row.getField("VENDOR_SCHOOL_ID"), row.getField(pType + "_VENDOR_SCHOOL_ID"));
                    }
                default:
    ...
    function mapAdditionalAdminFields(row, pType) {
        ...
        if(pType !== null){
            staffID = _util.trim(row.getField( pType + "_STAFF_CODE"));
            courseCode = row.getField(pType + "_COURSE_CODE");
            courseSection = row.getField(pType + "_COURSE_SECTION");
            teacherLastName = row.getField(pType + "_TEACHER_LAST_NAME");
            teacherFirstName = row.getField(pType + "_TEACHER_FIRST_NAME");
            teacherName = teacherLastName + ", " + teacherFirstName;
            partYear = row.getField(pType + "_SCHOOL_YEAR_ID");
            lep = row.getField(pType + "_LEP");
            sped = row.getField(pType + "_SPED").substring(0,1);
            rawAdminDateStr = row.getField(pType + "_TEST_ADMIN_DATE_STR");
            race = row.getField(pType + "_RACE_ETHNICITY_NAME");
            population = row.getField(pType + "_STUDENT_POPULATION");
            windStartDate = row.getField(pType + "_WINDOW_START_DATE");{
            if (windStartDate.toString().indexOf("-") > -1 && windStartDate!=='NULL') {
                windStartDate = AppUtil.getDateObjByPattern(windStartDate, "dd-MM-yyyy").STANDARD_DATE;
            }
            else if (windStartDate.toString().indexOf("/") > -1 && windStartDate!=='NULL') {
                windStartDate = AppUtil.getDateObjByPattern(windStartDate, "MM/dd/yyyy").STANDARD_DATE;
            }
            else{
                if(windStartDate!='NULL'){
    ...
    function mapScaledScore(row, testMetadata, pType, signature) {
		...
		var perfLevel = _util.coalesce(row.getField("TEST_PRIMARY_RESULT"), row.getField(pType+"_TEST_PRIMARY_RESULT"));
		var scaleScore = _util.coalesce(row.getField("TOTAL_NO_OF_POINTS_ACHIEVED"), row.getField( pType + "_TOTAL_NO_OF_POINTS_ACHIEVED"));
		var pointsPoss = _util.coalesce(row.getField("TEST_NO_OF_POINTS_POSSIBLE"), row.getField(pType + "_TEST_NO_OF_POINTS_POSSIBLE"));
        var percentage = _util.coalesce(row.getField("TEST_PERCENTAGE_SCORE"), row.getField(pType + "_TEST_PERCENTAGE_SCORE"));
        var passInd = _util.coalesce(row.getField("TEST_PASSED_INDICATOR"), row.getField(pType + "_TEST_PASSED_INDICATOR"));
        var eightyPlus = _util.coalesce(row.getField("80_PLUS"), row.getField(pType+"_80_PLUS"));
        var diff = _util.coalesce(row.getField("POST_PRE"));
        var impr = _util.coalesce(row.getField("IMPROVEMENT"));
        var subRecodeRea = _util.coalesce(row.getField("TEST_SUBJECT_RECODE_REA"), row.getField(pType + "_TEST_SUBJECT_RECODE_REA"));
        ...
        if(pType!==null){
                if(row.getField("PRE_MASTER_ID")!==null){
                    score["TEST_TERTIARY_RESULT_CODE"] = "PREID";
                    score["TEST_TERTIARY_RESULT"] =  row.getField("PRE_MASTER_ID");
                }
                if(row.getField("POST_MASTER_ID")!==null) {
                    score["TEST_QUATERNARY_RESULT_CODE"] = "POSID";
                    score["TEST_QUATERNARY_RESULT"] = row.getField("POST_MASTER_ID");
                }
        }
        else{
   ...
   function generateTestNumber(row, testMetadata, pType, signature){
        ...
        if(signature.getFormat().getFormatName() === "NJ_LOCAL_GENERIC_99_COLS"){
            if(row.getField(pType + "_MASTER_ID") !== 'NULL') {
                var testNumber = testMetadata.TEST + "_" + pType + "_" + row.getField(pType + "_MASTER_ID");
            }
        }
        else{
   ...
   function remove(testName){
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord, pType) {
        ...
        if(pType!==null){
            assessmentAdminKey = assessmentAdminKey + "_" + pType;
        }
