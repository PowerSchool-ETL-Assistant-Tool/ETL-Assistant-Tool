var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "NJGPA";
    ...
    var scoresToMap = [
        {TEST: "NJGPA", CODE:"OVR", FIELD: "OVERALL_", MAP_FUNCTION: mapOverallScore}
    ];
    ...
    "1": { TEST_RESULT: 'Level 1', PASS_IND: 'No'},
    "2": { TEST_RESULT: 'Level 2', PASS_IND: 'Yes'},
    null: { TEST_RESULT: null, PASS_IND: "--"},
    "": { TEST_RESULT: null, PASS_IND: "--"},
    undefined: { TEST_RESULT: null, PASS_IND: "--"}
    };
    ...
    // Check for null or undefined input
        ...
        if (lowerCaseInput.indexOf("fall") !== -1) {
             return "FALL";
        } else if (lowerCaseInput.indexOf("summer") !== -1) {
             return "SUMMER";
        } else if (lowerCaseInput.indexOf("spring") !== -1) {
             return "SPRING";
        } else if (lowerCaseInput.indexOf("winter") !== -1) {
             return "WINTER";
    }
    // Return null if no season match is found
              return null;
    }
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        var extractBirthDateDay = dateConverter(birthDateObject).days;
        var extractBirthDateMonth = dateConverter(birthDateObject).months;
        var extractBirthDateYear = dateConverter(birthDateObject).years;

        var finalDate = null;
        var testDate = row.getField("UNIT_2_TEST_DATE");
        if (testDate!==undefined && testDate!==null && testDate!== '')
         {
            finalDate = dateConverter(testDate).finalDates;
         } else {
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "REPORTING_PERIOD":
                    var rawPeriod = _util.coalesce(row.getField("REPORTING_PERIOD"), hierarchy.REPORTING_PERIOD);
                    record.REPORTING_PERIOD = getSeason(rawPeriod);
                    break;
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = AppUtil.getMonthFollowedByDay(finalDate, "MM/dd/yyyy").STANDARD_DATE;
                    break;
                ...
                case "STUDENT_DOB_MONTH":
                    record.STUDENT_DOB_MONTH = _util.coalesce(extractBirthDateMonth, null);
                    break;
                case "STUDENT_DOB_DAY":
                    record.STUDENT_DOB_DAY = _util.coalesce(extractBirthDateDay, null);
                    break;
                case "STUDENT_DOB_YEAR":
                    record.STUDENT_DOB_YEAR = _util.coalesce(extractBirthDateYear, null);
                    break;
                ...
                case "SCHOOL_LOCAL_ID":
                     record.SCHOOL_LOCAL_ID = _util.coalesce(row.getField("SCHOOL_NUMBER"), row.getField("SCHOOL_LOCAL_ID"));
                     break;
    ...
    function mapAdditionalAdminFields(row) {
        ...
        var admin = {};
        var prob = row.getField("STUDENTWITHDISABILITIES");
        admin.TEST_INTERVENTION_CODE = row.getField("ACCOUNTABLE_SCHOOL_CODE");
        admin.TEST_INTERVENTION_DESC = row.getField("ACCOUNTABLE_SCHOOL_NAME");
...
//        var testNumber = testMetadata.TEST + testMetadata.CODE;
        ...
        if(subject.indexOf("English") > -1) {
            testNumber = testMetadata.TEST + "_ELA_" + testMetadata.CODE;
        ...
        var learner = row.getField("ENGLISHLEARNEREL");
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord) {
        ...
        var testDate1 = srcRecord.getField("UNIT_2_TEST_DATE");
        var finalDate1 = null;
        if (testDate1!==undefined && testDate1!==null && testDate1!== '')
         {
            finalDate1 = dateConverter(testDate1).finalDates;
         } else {
               finalDate1 = "05/15/" + hierarchy.SCHOOL_YEAR.substring(5,9);
         }
         assessmentAdminKey = assessmentAdminKey + "_" + finalDate1
