var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "LAUSD_IREADY_SPANISH";
    ...
    var elaScoresToMap = [
        /*Reading Spanish Test Scores*/
        { MAP_FUNCTION: mapELASpanishScore, SUBJECT: 'RD_SPA', TEST_NUMBER_SUFFIX: 'PHON_AWARE', NAME: 'PHONOLOGICAL_AWARENESS' }
        , { MAP_FUNCTION: mapELASpanishScore, SUBJECT: 'RD_SPA', TEST_NUMBER_SUFFIX: 'PHONICS', NAME: 'PHONICS' }
        , { MAP_FUNCTION: mapELASpanishScore, SUBJECT: 'RD_SPA', TEST_NUMBER_SUFFIX: 'COMP_LIT', NAME: 'COMPREHENSION_LITERATURE' }
        , { MAP_FUNCTION: mapELASpanishScore, SUBJECT: 'RD_SPA', TEST_NUMBER_SUFFIX: 'COMP_INFO_TEXT', NAME: 'COMPREHENSION_INFORMATIONAL_TEXT' }
        , { MAP_FUNCTION: mapELASpanishScore, SUBJECT: 'RD_SPA', TEST_NUMBER_SUFFIX: 'VOCAB', NAME: 'VOCABULARY' }
        , { MAP_FUNCTION: mapELASpanishOverallScore, SUBJECT: 'RD_SPA', TEST_NUMBER_SUFFIX: 'OVERALL'}
    ];

    var mathScoresToMap = [
        /*Math Spanish Test Scores*/
        { MAP_FUNCTION: mapMathSpanishScore, SUBJECT: 'MATH_SPA', TEST_NUMBER_SUFFIX: 'NUM_OPS', NAME: 'NUMBER_AND_OPERATIONS' }
        , { MAP_FUNCTION: mapMathSpanishScore, SUBJECT: 'MATH_SPA', TEST_NUMBER_SUFFIX: 'ALG', NAME: 'ALGEBRA_AND_ALGEBRAIC_THINKING' }
        , { MAP_FUNCTION: mapMathSpanishScore, SUBJECT: 'MATH_SPA', TEST_NUMBER_SUFFIX: 'MEASURE', NAME: 'MEASUREMENT_AND_DATA' }
        , { MAP_FUNCTION: mapMathSpanishScore, SUBJECT: 'MATH_SPA', TEST_NUMBER_SUFFIX: 'GEO', NAME: 'GEOMETRY' }
        , { MAP_FUNCTION: mapMathSpanishScore, SUBJECT: 'MATH_SPA', TEST_NUMBER_SUFFIX: 'OVERALL', NAME: 'OVERALL' }
    ];
    ...
    function mapNormalizedRecord(signature, hierarchy, row, attempt) {
        ...
        //Map ELA score fields for each expected score mapping
        if(signature.format.formatName === "LAUSD_iReady_Spanish_ELA_23_24") {
            elaScoresToMap.forEach(function (testMetadata) {
                try {
        ...
        //Map Math score fields for each expected score mapping
        if(signature.format.formatName === "LAUSD_IREADY_SPANISH_MATH_69COLS") {
            mathScoresToMap.forEach(function (testMetadata) {
                try {
                    var score = (testMetadata.MAP_FUNCTION)(row, testMetadata, attempt,signature);
                    AppUtil.nullSafePush(mappingObject.SCORE_MAPPINGS, score);
                } catch (exception) {
        ...
        normalizedFileFields.forEach(function (field) {
            switch (field) {
                ...
                case"SCHOOL_STATE_ID":
                    ...
                    break;
                case "SCHOOL_NAME":
                    record.SCHOOL_NAME = _util.coalesce(row.getField("SCHOOL"), row.getField("SCHOOL_STATE_ID"));
                    break;
                default:
    ...
    function mapAdditionalAdminFields(row) {
        ...
        var admin = {};
        // Rush Flag Disabled for IREADY_SPANISH.
        //Note: Spanish ELA does not have capabilities for BaseLine Diagnostic, Most Recent Diagnostic and enrolled.
        //var code = "--";
        ...
        admin.TEST_EXEMPTION_CODE = null;
        admin.TEST_EXEMPTION_DESC = null;
       ...
       // var rushFlag = _util.coalesce(row.getField("RUSH_FLAG"), row.getField("NOTES"), "--");
        ...
        var baselineDiagnostic = row.getField("BASELINE_DIAG_YN");
        var mostRecentDiagnostic = row.getField("MOST_RECENT_0");
        var acquisition = row.getField("ENGLISH_LANGUAGE_ACQUISITION");
        if(acquisition !== undefined && acquisition !== null && acquisition !== ""){
            //Field English Language Acquisition added on 11/08/2024
            var sepMatch = acquisition.match(/(.*?)\s*[\(\[\{](.*?)[\)\]\}]/);
            var outsideBrackets = sepMatch[1].trim(); // Extracts and trims the part outside the brackets
            var insideBrackets = sepMatch[2].trim(); // Extracts and trims the part inside the brackets
            admin.TEST_EXEMPTION_CODE = insideBrackets;
            admin.TEST_EXEMPTION_DESC = outsideBrackets;
        }
        /*
    ...
    function mapELASpanishScore(row, testMetadata, attempt,signature) {
        var score = {};
        var testNumber = 'LA_IREADY_SPANISH_' + testMetadata.SUBJECT + '_' + testMetadata.TEST_NUMBER_SUFFIX;
        var scaleScore = row.getField(testMetadata.NAME + '_%_CORRECT');
        var placementScore = row.getField("OVERALL_SPANISH_PLACEMENT");
        ...
        (percentScore === null || percentScore === undefined || _util.trim(percentScore) == "")) {
            var testNumber = 'LA_IREADY_SPANISH_' + testMetadata.SUBJECT + '_' + testMetadata.TEST_NUMBER_SUFFIX;
            score["TEST_NUMBER"] = testNumber;
            score["TEST_PRIMARY_RESULT"] = "No Score";
            score["TEST_CUSTOM_RESULT"] = placementScore;
            return score;
        }
    ...
    function mapELASpanishOverallScore(row, testMetadata, attempt,signature) {
        var score = {};
        var testNumber = 'LA_IREADY_SPANISH_' + testMetadata.SUBJECT + '_' + testMetadata.TEST_NUMBER_SUFFIX;
        var placementScore = row.getField("OVERALL_SPANISH_PLACEMENT");
        if(placementScore === null || placementScore === undefined || _util.trim(placementScore) === "--" || _util.trim(placementScore) === "" ) {
            return null;
        }
        score["TEST_NUMBER"] = testNumber;
        score["TEST_PRIMARY_RESULT"] = placementScore;
    ...
    function mapMathSpanishScore(row, testMetadata, attempt,signature) {
        var score = {};
        if(row.getField("COMPLETION_DATE_" + attempt) !== "NULL"){
            if (row.getField("COMPLETION_DATE_" + attempt) !== null && row.getField("COMPLETION_DATE_" + attempt) !== undefined) {
                var format = getDateFormat(row.getField("COMPLETION_DATE_" + attempt));
        ...
        diagnosticLanguage = row.getField("DIAGNOSTIC_LANGUAGE");
        //Adding a global filter to remove all regular iready admins from spanish
        if((diagnosticLanguage !== null) && (diagnosticLanguage.toUpperCase() !== "SPANISH")){
            return null;
        } else {
            var subject = testMetadata.SUBJECT;
            ...
            var testNumber = 'LA_IREADY_SPANISH_' + subject + '_' + testMetadata.TEST_NUMBER_SUFFIX;
            ...
            if (testMetadata.TEST_NUMBER_SUFFIX == "OVERALL" && (subject == "MATH" || subject == "MATH_SPA" ) && row.getField("QUANTILE_MEASURE_0") !== null) {
            ...
            if (testMetadata.TEST_NUMBER_SUFFIX == "OVERALL") {
                ...
                if (row.getField("COMPLETION_DATE_" + attempt) !== null) {
                    score["TEST_SCORE_ATTRIBUTES"] = testDate;
                }
