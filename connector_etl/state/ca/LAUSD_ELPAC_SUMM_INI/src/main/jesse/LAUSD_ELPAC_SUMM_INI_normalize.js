var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "LAUSD_ELPAC_SUMM_INI";
    ...
    var scoresToMap =
        [
            ...
            {TEST: "CA_ELPAC", CODE: "OVR", FIELD:"", MAP_FUNCTION: mapCompositeScore}
            ,{TEST: "CA_ELPAC", CODE: "ORAL", FIELD: "ORAL_LANGUAGE", MAP_FUNCTION: mapScore}
            , {TEST: "CA_ELPAC", CODE: "WL", FIELD: "WRITTEN_LANGUAGE", MAP_FUNCTION: mapScore}
            , {TEST: "CA_ELPAC", CODE: "LIS", FIELD: "LISTENING", MAP_FUNCTION: mapStrandScore}
            , {TEST: "CA_ELPAC", CODE: "SPE", FIELD: "SPEAKING", MAP_FUNCTION: mapStrandScore}
            , {TEST: "CA_ELPAC", CODE: "RD", FIELD: "READING", MAP_FUNCTION: mapStrandScore}
            , {TEST: "CA_ELPAC", CODE: "WR", FIELD: "WRITING", MAP_FUNCTION: mapStrandScore}
            , {TEST: "CA_ELPAC", CODE: "RI", FIELD: "RESEARCH_AND_INQUIRY", MAP_FUNCTION: mapStrandScore}
            , {TEST: "CA_ELPAC", CODE: "CLS", FIELD: "CAST_LIFE", MAP_FUNCTION: mapStrandScore}
            , {TEST: "CA_ELPAC", CODE: "CPS", FIELD: "CAST_PHYSICAL", MAP_FUNCTION: mapStrandScore}
            , {TEST: "CA_ELPAC", CODE: "CEPS", FIELD: "CAST_EARTH_AND_SPACE", MAP_FUNCTION: mapStrandScore}
        ];
    ...
    var decodePerformanceLevel = {
        ...
        "BS" : {CODE: "BS", DESC: "Below Standard", PASS: 'No'},
        "NS" : {CODE: "NS", DESC: "Near Standard", PASS: 'No'},
        "AS" : {CODE: "AS", DESC: "Above Standard", PASS: 'Yes'}
    };
    ...
    function mapDateObject (hierarchy, row, dateStr) {
        ...
        if (adminDate === null && adminDate !== undefined) {
            try {
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        normalizedFileFields.forEach(function (field) {
            ...
            const dateFormat = mapDateObject(hierarchy, row, row.getField("SUBMIT_DATE_TIME"));
...
//                    break;
                ...
                case "STUDENT_STATE_CODE":
                    ...
                    record.STUDENT_STATE_CODE = _util.coalesce(row.getField("STUDENT_STATE_CODE"), "").replace("US-","");
                    break;
    ...
    function mapScore(row, testMetadata) {
        ...
        // Check for key fields and do not map score and exit if conditions met.
        if((scaleScore === null || scaleScore === undefined || _util.trim(scaleScore) === "--" || _util.trim(scaleScore) === "") && (performanceLevel === null || performanceLevel === undefined || _util.trim(performanceLevel) === "--" || _util.trim(performanceLevel) === "")){
            return null;
        }
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord) {
        ...
        try{
            var adminDateFormat = mapDateObject(hierarchy, srcRecord, srcRecord.getField("SUBMIT_DATE_TIME"));
            var testDateObject = {
                RAW_DATE : null
                , MONTH : null
                , DAY : null
            ...
            if (srcRecord.getField("TESTING_COMPLETED_DATE")!== null && srcRecord.getField("TESTING_COMPLETED_DATE")!== undefined && srcRecord.getField("TESTING_COMPLETED_DATE")!== ""){
                testDateObject = parseDate(srcRecord.getField("TESTING_COMPLETED_DATE"));
            }
            else {
                var dates = [];
                //Some students only have certain dates, only those should be included or it causes an undefined date object.
                if(srcRecord.getField("TESTING_COMPLETED_DATE_FINAL")!==null && srcRecord.getField("TESTING_COMPLETED_DATE_FINAL") !== ""){
                    dates.push(new Date(srcRecord.getField("TESTING_COMPLETED_DATE_FINAL")+"Z"));
                }
                if(srcRecord.getField("TESTING_COMPLETED_DATE_LISTENING")!==null && srcRecord.getField("TESTING_COMPLETED_DATE_LISTENING") !== ""){
                    dates.push(new Date(srcRecord.getField("TESTING_COMPLETED_DATE_LISTENING")+"Z"));
                }
                if(srcRecord.getField("TESTING_COMPLETED_DATE_READING")!==null && srcRecord.getField("TESTING_COMPLETED_DATE_READING") !== ""){
                    dates.push(new Date(srcRecord.getField("TESTING_COMPLETED_DATE_READING")+"Z"));
                }
                if(srcRecord.getField("TESTING_COMPLETED_DATE_SPEAKING")!==null && srcRecord.getField("TESTING_COMPLETED_DATE_SPEAKING") !== ""){
                    dates.push(new Date(srcRecord.getField("TESTING_COMPLETED_DATE_SPEAKING")+"Z"));
                }
                if(srcRecord.getField("TESTING_COMPLETED_DATE_WRITING")!==null && srcRecord.getField("TESTING_COMPLETED_DATE_WRITING") !== ""){
                    dates.push(new Date(srcRecord.getField("TESTING_COMPLETED_DATE_WRITING")+"Z"));
                }
                var maximumDate=new Date(Math.max.apply(null, dates));
        ...
        var testDate = _util.coalesce(adminDateFormat.STANDARD_DATE,testDateObject.STANDARD_DATE, '05/15/' + hierarchy.SCHOOL_YEAR.substring(5, 9));
    ...
    function generateTestNumber(row, testMetadata) {
        var testNumber = null;
        var recordType = _util.coalesce(row.getField("CONDITION_CODE"));
        var recordTypeAssessSep = _util.coalesce(row.getField("ASSESSMENT_TYPE"));

        // Check if score should be mapped as an alternate score
        if(recordType === "23" || recordType === "ALT") {
            //Assessment Separator in place
            if (recordTypeAssessSep  === "IA") {
