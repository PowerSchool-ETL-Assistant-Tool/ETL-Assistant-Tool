var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "OH_STATE";
    ...
    var scoresToMap = [
        {TEST: "OH-STATE", CODE:"OVR", MAP_FUNCTION: mapScaledScore}
    ];
    ...
    var perfLevelDecode = {
       "-" : { DESC:"Below", PASS_IND: "No"},
       "0" : { DESC:"Below", PASS_IND: "No"}
       ,"1" : { DESC:"Limited", PASS_IND: "No"}
       ,"2" : { DESC:"Basic", PASS_IND: "No"}
       ,"3" : { DESC:"Proficient", PASS_IND: "Yes"}
       ,"4" : { DESC:"Accomplished", PASS_IND: "Yes"}
       ,"5" : { DESC:"Advanced", PASS_IND: "Yes"}
       ,"A" : { DESC:"Advanced", PASS_IND: "Yes"}
       ,"N" : { DESC:"Not Proficient", PASS_IND: "No"}
       ,"P" : { DESC:"Proficient", PASS_IND: "Yes"}
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        var birthDateObject = AppUtil.getDateObjByPattern(row.getField("BIRTH_DATE"), "dd-MMM-yy");
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = formatTestDate(row);
                    break;
    ...
    function mapAdditionalAdminFields(row) {
        // TODO: Add or remove any additional admin fields that are non-standard to the case statement above.
        var admin = {};
     ...
     ***********************************************************************************/
    //TODO update scoring functions to ma what is set up in scoresToMap for the fields provided
    function mapScaledScore(row, testMetadata) {
        var score = {};
        var testType = row.getField("TEST_TYPE");
        var assessmentArea = row.getField("ASSESSMENT_AREA");
        var grade = row.getField("GRADE");
        var testNumber = testMetadata.TEST + "_" + testType + "_" + assessmentArea + "_" + gradeDecode[grade];
        var scaleScore = row.getField("SCALE_SCORE");
		var achievLevel = row.getField("PERFORMANCE_LEVEL");
		var gradelevel = row.getField("TEST_GRADE_LEVEL");

        // Check for key fields and do not map score and exit if conditions met.
        if((scaleScore === null || scaleScore === undefined || scaleScore === "***" || scaleScore === "P" || _util.trim(scaleScore) === "--" || _util.trim(scaleScore) === "")
         && (achievLevel === null || achievLevel === undefined  ||  achievLevel === "-" || _util.trim(achievLevel) === "")){
            return null;
        }
        ...
        if(testType === 'GA' || testType === 'GE' || testType ==='GF' || testType ==='GN' || testType === 'GW' || testType === 'GX' || testType === 'GY' || testType === 'GO' || testType === 'GB' || testType === 'GD' || testType === 'GM'){
    	score["TEST_PASSED_INDICATOR"] = perfLevelDecode[achievLevel].PASS_IND;
    	score["TEST_PRIMARY_RESULT_CODE"] = achievLevel;
    	score["TEST_PRIMARY_RESULT"] = perfLevelDecode[achievLevel].DESC;
    	}
    	score["TEST_GRADE_EQUIVALENT"] = gradelevel;
    ...
    function formatTestDate(row) {
            var year = row.getField("TEST_YEAR").substring(0, 4);
            year = year.substring(0, 4);
            var month = row.getField("TEST_YEAR").substring(4, 6);
            var day = row.getField("TEST_DAY");
            // Default to 15th if no day is found.
            if( day == "-" || day == null || _util.trim(day) == "") {
                day = "15";
