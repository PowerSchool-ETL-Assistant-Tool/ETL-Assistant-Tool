var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "ME_SCIENCE";
    ...
    var scoresToMap = [
        {TEST: "ME_SCI", CODE:"OVR", FIELD: "OVR", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "ME_SCI", CODE:"SUB_1", FIELD: "SUBSCORE_1_", MAP_FUNCTION: mapSubScore1}
        ,{TEST: "ME_SCI", CODE:"SUB_2", FIELD: "SUBSCORE_2_", MAP_FUNCTION: mapSubScore2}
        ,{TEST: "ME_SCI", CODE:"SUB_3", FIELD: "SUBSCORE_3_", MAP_FUNCTION: mapSubScore3}
    ];
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = _util.coalesce(testAdminDate, "05/15/"+hierarchy.SCHOOL_YEAR.substring(5));
                    break;
    ...
    function mapScaledScore(row, testMetadata) {
		...
		var minPoints = row.getField(testMetadata.FIELD + "_MINIMUM_POINTS_ON_TEST");
        var maxPoints = row.getField(testMetadata.FIELD + "_MAXIMUM_POINTS_ON_TEST");
    	...
    	if (!isNaN(minPoints)){
        score["TEST_ITEMS_POSSIBLE"] = minPoints;
        }
        score["TEST_ITEMS_ATTEMPTED"] = maxPoints;

        return score;
    }

    function mapSubScore1(row, testMetadata) {
        var score = {};
     //   var grade = gradeDecode[row.getField('GRADE')];
        var subj = getSubject(row, testMetadata);
        var testNumber = testMetadata.TEST + "_" + testMetadata.CODE + "_" + subj;
        var rawScore = row.getField(testMetadata.FIELD + "STUDENT_RAW_SCORE");
        var maxPoints = row.getField(testMetadata.FIELD + "MAX_POINTS");
    	...
    	if (!isNaN(maxPoints)){
    	score["TEST_ITEMS_POSSIBLE"] = maxPoints;
    	}
    ...
    function mapSubScore2(row, testMetadata) {
        var score = {};
      //  var grade = gradeDecode[row.getField('GRADE')];
        var subj = getSubject(row, testMetadata);
        var testNumber = testMetadata.TEST + "_" + testMetadata.CODE + "_" + subj;
        ...
        function mapSubScore3(row, testMetadata) {
        var score = {};
        //var grade = gradeDecode[row.getField('GRADE')];
        var subj = getSubject(row, testMetadata);
        var testNumber = testMetadata.TEST + "_" + testMetadata.CODE + "_" + subj;
        ...
        function getSubject(row, testMetadata) {
        var subject = row.getField(testMetadata.FIELD + "NAME");
        if(subject){
        if (subject.indexOf('Physical Science') > -1) {
               subject = 'PHY_SCI';
        } else if (subject.indexOf('Life Science') > -1) {
               subject = 'LIF_SCI';
        } else if (subject.indexOf('Earth and Space Science') > -1) {
               subject = 'ESS_SCI';
        }else if (subject.indexOf('Structure and Properties of Matter') > -1) {
               subject = 'SPM';
        }else if (subject.indexOf('Matter and Energy in Organisms and Ecosystems') > -1) {
               subject = 'MEOE';
        }else if (subject.indexOf("Earth's Systems and Space Systems: Stars and the Solar System") > -1) {
               subject = 'ESS_SSM';
        }
        ...
        function getAdminDate(row, hierarchy) {
            ...
            switch(true){
                    case (fileName.indexOf("FALL") > -1) : adminDate = "09/15/" + hierarchy.SCHOOL_YEAR.substring(0, 4);
                        break;
                    case (fileName.indexOf("WINTER") > -1) : adminDate = "01/15/" + hierarchy.SCHOOL_YEAR.substring(5);
                        break;
                    case (fileName.indexOf("SPRING") > -1) : adminDate = "05/15/" + hierarchy.SCHOOL_YEAR.substring(5);
                        break;
