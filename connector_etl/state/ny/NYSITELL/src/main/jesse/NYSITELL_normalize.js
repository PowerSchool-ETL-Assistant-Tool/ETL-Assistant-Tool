var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "NYSITELL";
    ...
    var scoresToMap = [
        {TEST: "NYSITELL", CODE:"ELA", FIELD: "ELA_", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "NYSITELL", CODE:"NY", FIELD: "NY_", MAP_FUNCTION: mapPerfLevel}
    ];
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_ADMIN_DATE":
                    var separatedDate = row.getField("TEST_ADMIN_DATE");
                    if(!separatedDate){
                        // default date
                        record.TEST_ADMIN_DATE = "05/15/" + hierarchy.SCHOOL_YEAR.substring(0,4);
                    } else {
                        record.TEST_ADMIN_DATE = dateConverter(separatedDate).finalDates;
                    }
                    break;
    ...
    function mapScaledScore(row, testMetadata) {
        var score = {};
        var assessmentId = row.getField("STATE_ASSESSMENT_ID");
        var testNumber = testMetadata.TEST + "_" + assessmentId + "_" + testMetadata.CODE;
        ...
        var achievmentCode = row.getField(testMetadata.FIELD + "ACHIEVEMENT_CODE");
        ...
        if(!isNaN(scaleScore) && scaleScore !== null){
            score["TEST_SCORE_VALUE"] = scaleScore.split(".")[0];
            score["TEST_SCALED_SCORE"] = scaleScore.split(".")[0];
        }

    	score["TEST_PRIMARY_RESULT_CODE"] = achievmentCode;
    ...
    function mapPerfLevel(row, testMetadata) {
       var score = {};
       var studentIdentifier = row.getField("SUBTEST_IDENTIFIER");
       var testNumber = testMetadata.CODE + "_" + studentIdentifier;
