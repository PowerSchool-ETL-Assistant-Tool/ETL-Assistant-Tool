var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "VA_BENCHMARK";
    ...
    var scoresToMap = [
        {TEST: "VA_BENCHMARK", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "VA_BENCHMARK", MAP_FUNCTION: mapPerfLevel}
    ];
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        if(STUDENT_NAME!== null && STUDENT_NAME !== "" && STUDENT_NAME !== undefined){
            if(STUDENT_NAME.indexOf(",") > -1){
                var STUDENT_LAST_NAME = STUDENT_NAME.split(",")[0].trim();
                var STUDENT_FIRST_NAME = STUDENT_NAME.split(",")[1].trim();
            }else{
                var STUDENT_LAST_NAME = STUDENT_NAME.split(" ")[0].trim();
                var STUDENT_FIRST_NAME = STUDENT_NAME.split(" ")[1].trim();
            }
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "SCHOOL_STATE_ID":
                     ...
                     break;
                case "SCHOOL_NAME":
                     record.SCHOOL_NAME = _util.coalesce(row.getField("SCHOOL_NAME"),row.getField ("DISTRICT_NAME"));
                     break;
    ...
    function mapScaledScore(row, testMetadata) {
        ...
        var pointsPossible = row.getField("POINTS_POSSIBLE");
        var perfLevel = row.getField("PERFORMANCE_LEVEL");
        if(perfLevel !== null && perfLevel !== "" && perfLevel !== undefined){
            perfLevel = perfLevel.toUpperCase();
        }
    	...
    	score["TEST_RAW_SCORE"] = parseFloat(pointsEarned);
    	scaleScore = scaleScore.replace("%","");
        if(!isNaN(scaleScore)){
            score["TEST_SCORE_VALUE"] = parseFloat(scaleScore);
            score["TEST_SCALED_SCORE"] = parseFloat(scaleScore);
        }
    ...
    function getTestNumber(row, testMetadata){
        ...
        var fileName =  flName.substring(flName.lastIndexOf('/')+1).replaceAll('%20','');

        var testID = _util.coalesce(row.getField("TEST_ID"), row.getField("TEST_TYPE"));
        var testNumber;

            testNumber = testMetadata.TEST + "_" + testID;
     ...
     function mapPerfLevel(row, testMetadata) {
         ...
         var testNumber = getTestNumber(row, testMetadata);
         var scaleScore = row.getField("TESTSCORE%");
         var totalPoint = row.getField("TOTAL_POINTS");
         var standard = row.getField("STANDARDS");
         var pointsEarned = row.getField("POINTSEARNED");
         ...
         if(!isNaN(totalPoint)){
             	score["TEST_READING_LEVEL"] = parseInt(totalPoint);
             	}
         if(!isNaN(pointsEarned)){
             	score["TEST_ITEMS_ATTEMPTED"] = parseInt(pointsEarned);
         }
         score["TEST_SECONDARY_RESULT"] = standard;
         return score;
     }
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord) {
        ...
        var testDate = AppUtil.getDateObjByPattern(srcRecord.getField("TEST_ADMIN_DATE"), "MM/dd/yyyy").STANDARD_DATE;
