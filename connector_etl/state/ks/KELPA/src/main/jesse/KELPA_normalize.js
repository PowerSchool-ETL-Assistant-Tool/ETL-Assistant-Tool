var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "KELPA";
    ...
    var scoresToMap = [
        {TEST: "KELPA", CODE:"LIST", FIELD: "LISTEN", FIELD2: "LISTENING", MAP_FUNCTION: mapScore}
        ,{TEST: "KELPA", CODE:"RD", FIELD: "READ", FIELD2: "READING", MAP_FUNCTION: mapScore}
        ,{TEST: "KELPA", CODE:"SP", FIELD: "SPEAK", FIELD2: "SPEAKING", MAP_FUNCTION: mapScore}
        ,{TEST: "KELPA", CODE:"WR", FIELD: "WRITE", FIELD2: "WRITE", MAP_FUNCTION: mapScore}
        ,{TEST: "KELPA", CODE:"COMP", FIELD: "COMPREHENSIVE", MAP_FUNCTION: mapTotalScore}
        ,{TEST: "KELPA", CODE:"OVR", FIELD: "OVERALL", MAP_FUNCTION: mapTotalScore}

        // Proficiency Determination
        ,{TEST: "KELPA", CODE:"PD", FIELD: "PROFICIENCY_DETERMINATION", MAP_FUNCTION: mapOverallProficiency}
    ];
    ...
    var esolDecode = {
         "0": "Not an ESOL eligible student"
        ,"1": " Title III Funded"
        ,"2": "State ESOL/Bilingual Funded"
        ,"3": "Both Title III and State ESOL/Bilingual Funded"
        ,"4": "Monitored ESOL student (status after scoring fluent on KELPA2 for two consecutive years)"
        ,"5": "ESOL program eligible, based on an English language proficiency test,but not currently receiving ESOL program services"
        ,"6": "Receives ESOL services and not funded with Title III and/or State ESOL Funding"
        ,"7": "Optional Transitional year"
        ,"8": "Monitored"
        ,"" : null
        ,null : null
    };
    ...
    function getTEST_ADMIN_DATE_STR(hierarchy) {
        ...
        return "05/15/" + schoolYear.substring(5, 9);
    }
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = row.getField("STATE_ID");
                    break;
                case "STUDENT_VENDOR_ID":
                    record.STUDENT_VENDOR_ID = row.getField("STATE_ID");
                ...
                case "STUDENT_GRADE_CODE":
                    record.STUDENT_GRADE_CODE = gradeDecode[row.getField('GRADE_CODE')];
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("BUILDING"),row.getField("SCHOOL_NAME"));
                    break;
//                case "SCHOOL_LOCAL_ID":
//                     record.SCHOOL_LOCAL_ID = _util.coalesce(row.getField("ORGANIZATION"),row.getField("SCHOOL_LOCAL_ID"));
//                     break;
...
//        admin.TEST_INTERVENTION_DESC = row.getField("INTERVENTION_CODE");
    ...
    function mapScore(row, testMetadata) {
        ...
        var value = row.getField(testMetadata.FIELD +"_STATUS");
        var perfLevel = row.getField(testMetadata.FIELD2 + "_PERFORMANCE_LEVEL");
        var esol = row.getField("ESOL_CODE");
        // Check for key fields and do not map score and exit if conditions met.
        if(scaleScore === null || scaleScore === undefined || _util.trim(scaleScore) === "--" || _util.trim(scaleScore) === "") {
            return null;
        }
        if(value!=='C' || value === null|| value === ''){
            return null;
        }
        // Set score fields
        ...
        if(!isNaN(scaleScore)){
         ...
         score["TEST_PRIMARY_RESULT"] = decodeProficiencyLevel[perfLevel];
         score["TEST_PRIMARY_RESULT_CODE"] = perfLevel;
         score["TEST_SCORE_ATTRIBUTES"] = esolDecode[esol];
         score["TEST_SECONDARY_RESULT_CODE"] = esol;
        return score;
    }
     ...
     var score = {};
         var testNumber = testMetadata.TEST + '_' + testMetadata.CODE;
         var achieveLevel = row.getField("PROFICIENCY_DETERMINATION");
         // Check for key fields and do not map score and exit if conditions met.
         if(achieveLevel === null || achieveLevel === undefined || _util.trim(achieveLevel) === "--" || _util.trim(achieveLevel) === "") {
             return null;
         }
         ...
         score["TEST_PRIMARY_RESULT"] = decodeProficiencyLevel[achieveLevel];
