var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "FAST_STAR";
    ...
    var scoresToMap = [
        ...
        {CODE: "FAST_STAR", FIELD: "EQUIVAL", MAP_FUNCTION: mapFastEquivalentScore},
    ...
    var perfLevels = {
        ...
        //TODO new decodes StateBenchmarkCategoryName
...
//
           ...
           // var perfObject = decodePerfLevel(_util.coalesce(row.getField("RENAISSANCEBENCHMARKCATEGORYNAME")));
            ...
            if (!isNaN(growth4)) {
            score["TEST_GROWTH_TARGET_4"] = row.getField("STUDENTGROWTHPERCENTILEFALLFALL");
            }
            ...
            if(spanishModifier==="SP"){
                ...
                score["TEST_PRIMARY_RESULT"] =  perfObject.RESULT;//_util.coalesce(row.getField("SCREENING_CATEGORY"), row.getField("SCREENING_CATEGORY_GROUP_ADJUSTMENT"), row.getField("SCREENINGCATEGORYGROUPADJUSTMENT"), row.getField("MASTERY_LEVEL"));
                score["TEST_PRIMARY_RESULT_CODE"] = perfObject.RESULT_CODE;
            }
            score["TEST_CUSTOM_RESULT_CODE"] = _util.coalesce(row.getField("STATEBENCHMARKCATEGORYADJUSTMENT"));
            rawGE = null;
    ...
    function mapBenchmarkScoreAssessmentBenchmarks(row, testMetadata) {
        try {
            ...
            score["TEST_PRIMARY_RESULT_CODE"] = "SAT";
            score["TEST_PRIMARY_RESULT"] = "SAT Prediction: " + row.getField("SATBENCHMARKCATEGORY");//_util.coalesce(row.getField("SCREENING_CATEGORY"), row.getField("SCREENING_CATEGORY_GROUP_ADJUSTMENT"), row.getField("SCREENINGCATEGORYGROUPADJUSTMENT"), row.getField("MASTERY_LEVEL"));
            score["TEST_SECONDARY_RESULT_CODE"] = "ACT";
            score["TEST_SECONDARY_RESULT"] = "ACT Prediction: " + row.getField("ACTBENCHMARKCATEGORY");
            return score;
        } catch(exception) {
    ...
    function mapFastEquivalentScore(row, testMetadata) {

            var scaleScore = row.getField("FAST_SCALED_SCORE");
            var grade = decodeGrade(row.getField("STUDENT_GRADE_CODE"));
            ...
            score["TEST_NUMBER"] = testMetadata.CODE + "_" + testMetadata.FIELD + "_" + grade;
            if (!isNaN(scaleScore)) {
                score["TEST_SCALED_SCORE"] = scaleScore;
                score["TEST_SCORE_VALUE"] = scaleScore;
            }
            ...
            score["TEST_PRIMARY_RESULT_CODE"] = benchmarkCategoryRecode[row.getField("ACHIEVEMENTLEVEL")];
            score["TEST_PRIMARY_RESULT"] = row.getField("ACHIEVEMENTLEVEL");
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
    	...
    	var grade = decodeGrade(_util.coalesce(row.getField("STUDENT_GRADE_CODE"), row.getField("GRADE"), row.getField("CURRENT_GRADE"), row.getField("CURRENTGRADE")));
    	 ...
    	 if(row.getField("STUDENTSTATEID")){
              var studentId = row.getField("STUDENTSTATEID").replace('FL', '').replace('X', '').replace(/^0+/, '');
          }
    	...
    	normalizedFileFields.forEach(function(field) {
    		switch (field) {
                ...
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = _util.coalesce(studentId,row.getField("STUDENT_STATE_ID"));
...
//        if(fileName.indexOf("SM_V2")>-1){
//             spanishModifier = "SP";
//        }
        if (spanishModifier !== null) {
            return `FAST_STAR_${testType}${spanishModifier}${subject}${grade}${scoreType}`;
        }
        return `FAST_STAR_${testType}${subject}${grade}${scoreType}`;
    }
    ...
    function determineFileSubject(format,normalizedFile) {
        ...
        if (fileName.indexOf('Star_Early_Literacy')> -1) {
            subject = 'LIT';
        } else if ((fileName.indexOf('Star_Reading')> -1 || fileName.indexOf('Star_English')> -1 || fileName.indexOf('STAR_READING')> -1) || fileName.indexOf('STAR_Reading')> -1) {
            subject = 'RD';
        } else if ((fileName.indexOf('Star_Math')> -1 || fileName.indexOf('STAR_MATH')> -1 || fileName.indexOf('STAR_Math')> -1)) {
            subject = 'MA';
        } else if (fileName.indexOf('Star_CAT_MATH')> -1) {
            subject = 'MA';
        }else if (fileName.indexOf('SR_V2')> -1){
           subject = 'RD';
        }else if (fileName.indexOf('SR_v2')> -1){
           subject = 'RD';
        }else if (fileName.indexOf('SM_v2')> -1){
           subject = 'MA';
        }else {
