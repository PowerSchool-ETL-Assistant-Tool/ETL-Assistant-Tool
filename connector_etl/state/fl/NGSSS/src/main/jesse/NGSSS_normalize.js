var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "NGSSS";
    ...
    var scoresToMap = [
        {TEST: "NGSSS", CODE:"_OVR", FIELD: "OVERALL_", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "NGSSS", CODE:"_PTPOSS_1", FIELD: "_POS1", MAP_FUNCTION: mapItemScore}
        ,{TEST: "NGSSS", CODE:"_PTPOSS_2", FIELD: "_POS2", MAP_FUNCTION: mapItemScore}
        ,{TEST: "NGSSS", CODE:"_PTPOSS_3", FIELD: "_POS3", MAP_FUNCTION: mapItemScore}
        ,{TEST: "NGSSS", CODE:"_PTPOSS_4", FIELD: "_POS4", MAP_FUNCTION: mapItemScore}
        ,{TEST: "NGSSS", CODE:"_EARN_1", FIELD: "_EARN1", MAP_FUNCTION: mapItemScore}
        ,{TEST: "NGSSS", CODE:"_EARN_2", FIELD: "_EARN2", MAP_FUNCTION: mapItemScore}
        ,{TEST: "NGSSS", CODE:"_EARN_3", FIELD: "_EARN3", MAP_FUNCTION: mapItemScore}
        ,{TEST: "NGSSS", CODE:"_EARN_4", FIELD: "_EARN4", MAP_FUNCTION: mapItemScore}
    ];
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        var testDate = _util.coalesce(row.getField("TEST_ADMIN_DATE"));
        ...
        if(row.getField("STATE_STUDENT_ID")){
            var stateId = row.getField("STATE_STUDENT_ID").replace('FL', '').replace(/^0+/, '');
        }
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("SCH_CODE"), row.getField("DIS_CODE"), "DISTRICT");
                    break;
                case "SCHOOL_LOCAL_ID":
                     record.SCHOOL_LOCAL_ID = _util.coalesce(row.getField("SCH_CODE"), row.getField("DIS_CODE"));
                     break;
                case "SCHOOL_STATE_ID":
                     record.SCHOOL_STATE_ID = _util.coalesce(row.getField("DIS_CODE"));
                     break;
    ...
    function mapScaledScore(row, testMetadata) {
        var score = {};
        //Customer Confirmed that if no grades then reject the student
        if(row.getField("GRADE")=== null || row.getField("GRADE")=== ''){
            return null;
        }
        ...
        if(perfLevel === '0'){
            ...
            score["TEST_PASSED_INDICATOR"] = perfPassedDecode[perfLevel];
        }
        return score;
    }
    ...
    function generateTestNumber(row, testMetadata) {
        var subject;
        var testName = row.getField("TEST_NAME").toUpperCase();
        if(testName.indexOf('BIOLOGY') > -1){
            subject = '_BIO';
        } else if(testName.indexOf('CIVICS') > -1){
            subject = '_CIV';
        } else if(testName.indexOf('HISTORY') > -1){
            subject = '_HIST';
