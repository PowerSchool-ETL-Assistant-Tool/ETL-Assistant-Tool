var AppNormalize = (function () {
    ...
    var _assessmentIdentifier = "FSA_EOC";
     ...
     ***************************************************************************/
    var scoresToMap = [
        {TEST: "FSA", CODE:"_OVR", FIELD: "OVERALL_", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "FSA", CODE:"_PTPOSS_1", FIELD: "_POS1", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_PTPOSS_2", FIELD: "_POS2", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_PTPOSS_3", FIELD: "_POS3", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_PTPOSS_4", FIELD: "_POS4", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_PTPOSS_5", FIELD: "_POS5", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_EARN_1", FIELD: "_EARN1", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_EARN_2", FIELD: "_EARN2", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_EARN_3", FIELD: "_EARN3", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_EARN_4", FIELD: "_EARN4", MAP_FUNCTION: mapItemScore}
        ,{TEST: "FSA", CODE:"_EARN_5", FIELD: "_EARN5", MAP_FUNCTION: mapItemScore}
    ];
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        let schoolYear = _util.coalesce(hierarchy.SCHOOL_YEAR);

        var testDate = row.getField("TEST_DATE");

        if(testDate !== null && testDate !== undefined && testDate !== ""){
            if(testDate.length >= 8 && testDate.indexOf("/") > -1){
        ...
        if(row.getField("STUDENT_ID")){//Removing leading zeroes from FLEID
            var studentId = row.getField("STUDENT_ID").replace('FL', '').replace(/^0+/, '');
        }

       var grade = _util.coalesce(row.getField("GRADE"), row.getField("SGRADE"), row.getField("STUDENT_GRADE"));
        ...
        module.normalizedFileFields.forEach(function (field) {
            switch (field) {
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = _util.coalesce(testDate, getTestAdminDate(schoolYear.substr(5), assessmentPeriod));
                    break;

                //Student Information
                case "STUDENT_LOCAL_ID":
                    record.STUDENT_LOCAL_ID = _util.coalesce(
                        row.getField("DISUSE"), row.getField("FLEID")
                    );
                    break;
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = _util.coalesce(
                        studentId, row.getField("FLEID")
                    );
                    break;
                ...
                case "STUDENT_GRADE_CODE":
                    record.STUDENT_GRADE_CODE = GRADE_DECODE[_util.trim(grade)];
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("SCHOOL"),row.getField("SCHOOL_NAME"),row.getField("DISUSE"));
                    break;
     ...
     ***********************************************************************************/
    function mapScaledScore(row) {
        let score = {};
        var periodA = assessPeriod(row);

        let testGrade = _util.trim(row.getField("TGRADE"));

        let testNumber = generateTestNumber(row,periodA);
        ...
        if(testGrade !== null && testGrade !== undefined && testGrade !== "" ){
            if(testGrade <= '12'){
                score.TEST_GRADE_EQUIVALENT = GRADE_DECODE[testGrade];
            }
        }
    ...
    function assessPeriod(row){
        var admin = row.getField("ADMINISTRATION")
    ...
    function mapItemScore(row, testMetadata) {
        ...
        if(testName.indexOf('Alg 1') > -1 || testName.indexOf('ALG 1') > -1 ){
            subjectName = 'ALG1';
        } else if(testName.indexOf('Alg 2') > -1 || testName.indexOf('ALG 2') > -1){
            subjectName = 'ALG2';
        } else if(testName.indexOf('ELA') > -1){
            subjectName = 'ELA';
        } else if(testName.indexOf('Geom') > -1 || testName.indexOf('GEOM') > -1){
            subjectName = 'GEOM';
        } else if(testName.indexOf('Math') > -1 || testName.indexOf('MATH') > -1){
            subjectName = 'MATH';
        }

        var testNumber =  'FSA_EOC_' + subjectName  + testMetadata.CODE;

        var ptPoss = row.getField("PT" + testMetadata.FIELD);
        var earn = row.getField("ITEM" + testMetadata.FIELD);
    ...
    function generateTestNumber(row, periodA) {
        ...
        if(subject !== null && subject !== undefined && subject !== "")
        {
            ...
            if(subject.indexOf("MATHEMATICS") > -1 || subject.indexOf("MATH") > -1) {
                subName = "MATH";
            } else if(subject.indexOf("GEOMETRY") > -1 || subject.indexOf("GEOM") > -1) {
                subName = "GEOM";
            } else if(subject.indexOf("CIVICS") > -1 ) {
                subName = "CIV";
            } else if(subject.indexOf("ELA") > -1 ) {
                subName = "ELA";
            } else if(subject.indexOf("U.S. HISTORY") > -1) {
                subName = "HIST";
            } else if(subject.indexOf("BIOLOGY 1") > -1) {
                subName = "BIO1";
            }else if(subject.indexOf("ALGEBRA 1") > -1 || subject.indexOf("ALG 1") > -1) {
                subName = "ALG1";
            } else if(subject.indexOf("ALGEBRA 2") > -1 || subject.indexOf("ALG 2") > -1) {
                subName = "ALG2";
            } else if(subject.indexOf("ALGEBRA RETAKE") > -1) {
                subName = "ALG_RE";
            }
        }
        else {
        ...
        if(testName != null){
         ...
         //print("Hello")
        if(testName){
            return 'FSA_EOC_' + tName + '_' + periodA;
        } else {
             return 'FSA_EOC_' + subName + '_' + GRADE_DECODE[grade] + '_' + periodA;
        }
    }
