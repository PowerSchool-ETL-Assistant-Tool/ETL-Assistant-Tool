var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "ACCTEST";
    var _thisLineageSource = "Edvantage ${_assessmentIdentifier} Loader v1.0";
    var multipleRowsPerGroupingExpected = false;
    var header = ["reporting_year","collection_code","school_code","student_id","test_date","test_id","test_school","exemption_code","score","achievement_level","percentile","grade","accom_list","last_name","first_name"];
    /**
    ...
    module.createNormalizedFile = function(file, signature) {
		try {
           ...
           if(signature.getFormat().getHasHeader()){
              ...
              df.input("INPUT", _io.delim_input_ext(file.getFilePath(), "\t", "\"", "\\", header,true));
           }
    ...
    var scoresToMap = [
        {TEST: "ACCTEST", CODE:"TOT", FIELD: "", MAP_FUNCTION: mapScaledScore}
    ];
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = AppUtil.getDateObjByPattern(row.getField("TEST_DATE"), "yyyyMMdd").STANDARD_DATE;
                    break;
    ...
    function mapScaledScore(row, testMetadata) {
        var score = {};
        var testNumber = testMetadata.TEST + "_" + row.getField("TEST_CODE");
        ...
        score["TEST_PRIMARY_RESULT_CODE"] = removeStringChar(achievLevel);
        if(!isNaN(achievLevel)){
    	    score["TEST_PRIMARY_RESULT"] = achDecode[achievLevel];
    	}
    	else
    ...
    function removeStringChar(val) {
        if(val !== null && val !== undefined && val !== "") {
            if(!isNaN(val)) {
                return val;
            } else if(val === "Not Proficient") {
                return 0;
            }
            else {
