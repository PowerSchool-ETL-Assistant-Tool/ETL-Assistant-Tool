var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "MT_MSA";
     ...
     ***************************************************************************/
     ...
     var scoresToMap = [
          {TEST: "MSA", CODE:"SCI_TOT", FIELD: "SS_TOT", MAP_FUNCTION: mapScaleScore}
      ];
     ...
     */
        ...
        function dateConverter(dateString) {
            ...
            if (dateString.match(/^\d{4}\/\d{2}\/\d{2}$/)) { // YYYY/MM/DD format
    ...
    function mapScaleScore(row, testMetadata) {
        ...
        if (subjectName === 'SCI_ALT' || subjectName === 'SCI') {
                testNumber = testMetadata.TEST + "_" + subjectName;
            } else {
                return null; // Return null to indicate invalid test number
            }

        var scaleScore = _util.trim(row.getField("SS_TOT"));
...
//         Check for key fields and do not map score and exit if conditions met.
        ...
        function getSubjectFromFilename(row) {
            ...
            if (fileName.indexOf("SCIENCE_ALT") > -1) {
                subject = "SCI_ALT";
