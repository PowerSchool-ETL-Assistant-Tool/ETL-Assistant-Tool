var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "ACCESS";
     ...
     ***************************************************************************/
     ...
     var scoresToMap = [
             { TEST: "IL_ACCESS", CODE: "COMP", FIELD: "COMPREHENSION", MAP_FUNCTION: mapScaledScore }
             ,{ TEST: "IL_ACCESS", CODE: "LIS", FIELD: "LISTENING", MAP_FUNCTION: mapScaledScore }
             ,{ TEST: "IL_ACCESS", CODE: "LIT", FIELD: "LITERACY", MAP_FUNCTION: mapScaledScore }
             ,{ TEST: "IL_ACCESS", CODE: "ORAL", FIELD: "ORAL", MAP_FUNCTION: mapScaledScore }
             ,{ TEST: "IL_ACCESS", CODE: "RD", FIELD: "READING", MAP_FUNCTION: mapScaledScore }
             ,{ TEST: "IL_ACCESS", CODE: "SP", FIELD: "SPEAKING", MAP_FUNCTION: mapScaledScore }
             ,{ TEST: "IL_ACCESS", CODE: "WR", FIELD: "WRITING", MAP_FUNCTION: mapScaledScore }
             ,{ TEST: "IL_ACCESS", CODE: "TOT", FIELD: "OVERALL", MAP_FUNCTION: mapScaledScore }
             ,{ TEST: "IL_ACCESS", CODE: "TOT", FIELD: "COMPOSITE", MAP_FUNCTION: mapScaledScore }
        ];
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("SCHOOL_NUMBER"), row.getField("SCHOOL_LOCAL_ID"),row.getField("SCHOOL_STATE_ID"));
                    break;
   ...
   function mapScaledScore(row, testMetadata) {
          ...
          var tier = row.getField(testMetadata.FIELD + "_TIER");
       	  var proficiencyLevel = row.getField(testMetadata.FIELD + "_PL");
       	  var cluster = row.getField(testMetadata.FIELD + "_CLUSTER");
          var percentile = row.getField("PY_TEST");
               var type = row.getField("EXAM_TYPE")


               // Check for key fields and do not map score and exit if conditions met.
          if (
             (scaleScore === null || scaleScore === undefined || _util.trim(scaleScore) === "--" || _util.trim(scaleScore) === "") &&
             (proficiencyLevel === null || proficiencyLevel === undefined || _util.trim(proficiencyLevel) === "--" || _util.trim(proficiencyLevel) === "") &&
             (cluster === null || cluster === undefined || _util.trim(cluster) === "--" || _util.trim(cluster) === "")
              ) {
              return null;
          }
            ...
            score.TEST_PRIMARY_RESULT = decodeProficiencyLevel(proficiencyLevel);
           	score["TEST_SECONDARY_RESULT"] = type

           	score["TEST_PERCENTILE_SCORE"] = percentile;
           	score["TEST_QUATERNARY_RESULT"] = tier;
           	score["TEST_READING_LEVEL"] = cluster;
