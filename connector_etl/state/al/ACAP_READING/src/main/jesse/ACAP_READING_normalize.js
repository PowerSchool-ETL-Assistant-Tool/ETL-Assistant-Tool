var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "ACAP_READING";
    ...
    var scoresToMap = [
        {TEST: "ACAP_READING", CODE:"_ELA_OVR", FIELD: "ELA_", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "ACAP_READING_SUPPLIMENT", CODE:"_ELA_RD", FIELD: "READING_", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "ACAP_READING", CODE:"_ELA_RD", FIELD: "READING_", MAP_FUNCTION: mapRawScore}
        ,{TEST: "ACAP_READING_SUPPLIMENT", NUMBER: "RC", FIELD: "", MAP_FUNCTION: mapItemScore}
    ];
    ...
    var passDecode = {
      ...
      var performanceLevelDecode = {
        ...
        ,"2" : { DESC:"On Track", PASS_IND: "Yes"}
        ,"3" : { DESC:"Prepared", PASS_IND: "Yes"}
        ,"4" : { DESC:"Prepared", PASS_IND: "Yes"}
        ,"" : { DESC: null, PASS_IND: null}
        ,null : { DESC: null, PASS_IND: null}
      };
      var progressLevelDecode = {
           "0" : { DESC:"--"}
          ,"1" : { DESC:"Support Needed"}
          ,"2" : { DESC:"On Track"}
          ,"3" : { DESC:"Prepared"}
          ,"" : { DESC: null}
          ,null : { DESC: null}
        };
        var readingStatusDecode = {
           "1" : "Below Grade Level"
          ,"2" : "On or Above Grade Level"
          ,"" :  null
          ,null :  null
        };
    /***********************************************************************************
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        var finalDate = null;
        if(row.getField("SCHOOL_YEAR")!==null && row.getField("SCHOOL_YEAR")!==undefined){
            finalDate =  "05/15/" + row.getField("SCHOOL_YEAR");
        }
        else{
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("SCHOOL_ID"), row.getField("SCHOOL_LOCAL_ID"),row.getField("SCHOOL_STATE_ID"), row.getField("SCHOOL_NAME"));
                    break;
    ...
    function mapScaledScore(row, testMetadata) {
		...
		var achievLevel = row.getField(testMetadata.FIELD + "PERFORMANCE_LEVEL");
	    var statePerRank = row.getField(testMetadata.FIELD + "STATE_PERCENTILE_RANK");
    	var growthLevelCode = row.getField(testMetadata.FIELD + "GROWTH_LEVEL_CODE");
    	var lexScore = row.getField(testMetadata.FIELD + "LEXILESCORE");
        var quartileScore = row.getField(testMetadata.FIELD + "QUANTILE_SCORE");
        var readingStatus = row.getField(testMetadata.FIELD + "STATUS");
        ...
        score["TEST_QUARTILE_SCORE"] = quartileScore;
        score["TEST_SECONDARY_RESULT_CODE"] = readingStatus;
        score["TEST_SECONDARY_RESULT"] = readingStatusDecode[readingStatus];
    	score["TEST_PASSED_INDICATOR"] = performanceLevelDecode[_util.trim(achievLevel)].PASS_IND;
    	score["TEST_PRIMARY_RESULT_CODE"] = achievLevel;
    	score["TEST_PRIMARY_RESULT"] = performanceLevelDecode[_util.trim(achievLevel)].DESC;
    ...
    function mapRawScore(row, testMetadata) {
        ...
        var lexile = row.getField(testMetadata.FIELD + "LEXILE_SCORE");
        var progressLevel = row.getField(testMetadata.FIELD + "PROGRESS_LEVEL");
        var sufficiencyLevel = row.getField(testMetadata.FIELD + "SUFFICIENCY_PRELIM");
        var readingStatus = row.getField("READING_STATUS");
        // Check for key fields and do not map score and exit if conditions met.
        ...
        score["TEST_PRIMARY_RESULT_CODE"] = progressLevel;
        score["TEST_PRIMARY_RESULT"] = progressLevelDecode[_util.trim(progressLevel)].DESC;
        score["TEST_SECONDARY_RESULT"] = sufficiencyLevel;
        score["TEST_TERTIARY_RESULT_CODE"] = readingStatus;
        score["TEST_TERTIARY_RESULT"] = readingStatusDecode[_util.trim(readingStatus)];
    ...
    function mapItemScore(row, testMetadata, rcNumber) {
        ...
        if(row.getField("CONTENT_AREA")){
        var contentArea = _util.trim(row.getField("CONTENT_AREA").toUpperCase());
        if(contentArea === "READING"){
            var CA = "RD_"
        }
        }

        var testNumber = testMetadata.TEST + "_" + CA + testMetadata.NUMBER + rcNumber;
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord) {
        if(signature.getFormat().getFormatName() === "ACAP_READING_SUPPLIMENT_38C"){
            _assessmentIdentifier = "ACAP_READING_SUPPLIMENT"
        }
