var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "ACAP_SUMMATIVE";
    ...
    var scoresToMap = [
        {TEST: "ACAP_SUM_", CODE:"OVR", FIELD: "OVERALL_", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "ACAP_SUM_", NUMBER: "RC", FIELD: "", MAP_FUNCTION: mapItemScore}
        ,{TEST: "ACAP_SUM_", CODE:"READ_OVR", FIELD: "READING_", MAP_FUNCTION: mapScaledScore}
    ];
    ...
    var achLvlDecode = {
       "TRUE" : { DESC:"Lowest Achievement level", PASS_IND: "Yes"}
      ,"FALSE" : { DESC:"Not Lowest", PASS_IND: "No"}
      ,"" : null
      ,null : null
    };
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = _util.coalesce(row.getField("STUDENT_ID"),row.getField("STATE_STUDENT_ID"));
                    break;
    ...
    function mapScaledScore(row, testMetadata) {
		...
		var achLvl = row.getField(testMetadata.FIELD + "ACHIEVEMENT_LEVEL");
        ...
        ((scaleScore === null || scaleScore === undefined || _util.trim(scaleScore) === "--" || _util.trim(scaleScore) === "")
                && (achLvl === null || achLvl === undefined || _util.trim(achLvl) === "--" || _util.trim(achLvl) === ""))
        )
        ...
        if(achLvl != null){
        var ar = achLvlDecode[achLvl].PASS_IND;
        score["TEST_SECONDARY_RESULT_CODE"] = achLvlDecode[achLvl].DESC;
        score["TEST_SECONDARY_RESULT"] = achLvl;
        score["TEST_CUSTOM_PASS_IND"] = ar.substring(0,5);
        }
    ...
    function mapItemScore(row, testMetadata, rcNumber) {
        var score = {};
         ...
         if(row.getField("CONTENT_AREA")!= null){
        var contentArea = _util.trim(row.getField("CONTENT_AREA").toUpperCase());
        }
