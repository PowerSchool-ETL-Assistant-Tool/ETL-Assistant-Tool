var AppNormalize = (function() {
    ...
    var scoresToMap = [
        {TEST: "VT_SBA", CODE:"OVR", FIELD:"OVERALL", MAP_FUNCTION: mapScaledScore},
    ...
    var primaryDecode = {
     "Level 1" : {RESULT: "Beginning", CODE:"1", IND:"No"}
     ,"Level 2" : {RESULT: "Approaching", CODE:"2", IND:"No"}
     ,"Level 3" : {RESULT: "Meets", CODE:"3", IND:"Yes"}
     ,"Level 4" : {RESULT: "Exceeds", CODE:"4", IND:"Yes"}
     ,"Insufficient to score" : {RESULT: "Insufficient to score", CODE:"IS", IND:null}
     ,"N/A" : {RESULT:null, CODE:null, IND:null}
     ,"" : {RESULT: null, CODE:null, IND:null}
     ,null : {RESULT: null, CODE:null, IND:null}
    };
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        if (row.getField('STUDENT_NAME')) {
         ...
         if(row.getField("ENROLLED_SCHOOL")&& row.getField("ENROLLED_SCHOOL")!== "N/A"){
              var schoolNameId = row.getField("ENROLLED_SCHOOL");
              var schoolDisId = schoolNameId.split("(").pop().replace(')', '');
              var schoolId = schoolDisId.split("-")[1];
              var schName = schoolNameId.split("(")[0];
        }
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(schoolId,schName,"DISTRICT");
                    break;
                case "SCHOOL_NAME":
                     record.SCHOOL_NAME = row.getField("ENROLLED_SCHOOL");
                     break;
                case "SCHOOL_LOCAL_ID":
                     record.SCHOOL_LOCAL_ID = _util.coalesce(schoolId,schName,"DISTRICT");
                      break;
                default:
    ...
    function mapScaledScore(row, testMetadata) {
        ...
        var subject = decodeSubject[row.getField("TEST_SUBJECT")];

        var testNumber = testMetadata.TEST + "_" + subject;

        var scaleScore = row.getField("SCALE_SCORE");
        var sem = row.getField("SCALE_SCORE_STANDARD_ERROR");
		var achievLevel = row.getField("PERFORMANCE");
    	...
    	score["TEST_PRIMARY_RESULT"] = primaryDecode[achievLevel].RESULT;
    	score["TEST_PASSED_INDICATOR"] =primaryDecode[achievLevel].IND;
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord) {
        ...
        var grade =   gradeDecode[srcRecord.getField("TEST_GRADE")];
