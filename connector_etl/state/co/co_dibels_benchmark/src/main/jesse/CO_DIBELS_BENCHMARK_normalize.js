var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "CO Dibels Benchmark";
...
//            print("\nsignature data: ${signature}\n");
            ...
            _dataflow.create("RECORD BUILD")
                ...
                .transform("SCORE COUNT CHECK", ["RECORDS"], function (output, row) {
                    ...
                    // Report when no scores are found.
                        ...
                        AssessmentLoader.audits.push(AppAudit.writeRejectRecord(file.getFilePath(), "WARNING",row.LINEAGE_LINE, "No scores found or No test admin date.", JSON.stringify(row)));
                        } else {
    ...
    ***************************************************************************/
    ...
    var strandsAndFields =
        [
            ...
            // DIBELS NEXT STRANDS
            {TEST: "DIBNXT", STRAND: "FSF"    , FIELD: "ASSESSMENT_MEASURE-FSF"},
            {TEST: "DIBNXT", STRAND: "LNF"    , FIELD: "ASSESSMENT_MEASURE-LNF"},
            {TEST: "DIBNXT", STRAND: "PSF"    , FIELD: "ASSESSMENT_MEASURE-PSF"},
            {TEST: "DIBNXT", STRAND: "NWFCLS" , FIELD: "ASSESSMENT_MEASURE-NWF_(CLS)"},
            {TEST: "DIBNXT", STRAND: "NWFWWR" , FIELD: "ASSESSMENT_MEASURE-NWF_(WWR)"},
            {TEST: "DIBNXT", STRAND: "DORFWC" , FIELD: "ASSESSMENT_MEASURE-DORF-WORDSCORRECT"},
            //{TEST: "DIBNXT", STRAND: "DORFERR", FIELD: "DORF-ERRORS"},
            {TEST: "DIBNXT", STRAND: "DORFACC", FIELD: "ASSESSMENT_MEASURE-DORF_(ACCURACY)"},
            {TEST: "DIBNXT", STRAND: "DORFRT" , FIELD: "ASSESSMENT_MEASURE-DORF_(RETELL)"},
            {TEST: "DIBNXT", STRAND: "DORFRQ" , FIELD: "ASSESSMENT_MEASURE-DORF_(RETELL_QUALITY)"},
            {TEST: "DIBNXT", STRAND: "DAZECOR", FIELD: "ASSESSMENT_MEASURE-DAZE_(CORRECT)"},
            {TEST: "DIBNXT", STRAND: "DAZEINC", FIELD: "ASSESSMENT_MEASURE-DAZE_(INCORRECT)"},
            {TEST: "DIBNXT", STRAND: "DAZEADJ", FIELD: "ASSESSMENT_MEASURE-DAZE"},
            {TEST: "DIBNXT", STRAND: "TOT"    , FIELD: "ASSESSMENT_MEASURE-COMPOSITE_SCORE"},

            {TEST: "DIBNXT", STRAND: "ORFWC" , FIELD: "ASSESSMENT_MEASURE-ORF-WORDSCORRECT"},
            {TEST: "DIBNXT", STRAND: "ORFACC", FIELD: "ASSESSMENT_MEASURE-ORF_(ACCURACY)"},

            {TEST: "DIBNXT", STRAND: "ORFFLU", FIELD: "ASSESSMENT_MEASURE-ORF_(FLUENCY)"},

            {TEST: "DIBNXT", STRAND: "ORFRT" , FIELD: "ASSESSMENT_MEASURE-ORF_(RETELL)"},
            {TEST: "DIBNXT", STRAND: "ORFRQ" , FIELD: "ASSESSMENT_MEASURE-ORF_(RETELL_QUALITY)"},
            {TEST: "DIBNXT", STRAND: "MAZECOR", FIELD: "ASSESSMENT_MEASURE-MAZE_(CORRECT)"},
            {TEST: "DIBNXT", STRAND: "MAZEINC", FIELD: "ASSESSMENT_MEASURE-MAZE_(INCORRECT)"},
            {TEST: "DIBNXT", STRAND: "MAZEADJ", FIELD: "ASSESSMENT_MEASURE-MAZE"},

            {TEST: "DIBNXT", STRAND: "WRF"    , FIELD: "ASSESSMENT_MEASURE-WRF"},
            {TEST: "DIBNXT", STRAND: "NWFWRC" , FIELD: "ASSESSMENT_MEASURE-NWF_(WRC)"},

            {TEST: "DIBNXT", STRAND: "NWFVOCAB" , FIELD: "ASSESSMENT_MEASURE-VOCABULARY"},
            {TEST: "DIBNXT", STRAND: "NWFSPEL" , FIELD: "ASSESSMENT_MEASURE-SPELLING"},
            {TEST: "DIBNXT", STRAND: "NWFRAN" , FIELD: "ASSESSMENT_MEASURE-RAN"},
            {TEST: "DIBNXT", STRAND: "NWFRISK" , FIELD: "ASSESSMENT_MEASURE-RISK_INDICATOR"}
    ...
    var perfLevels =
        {
            ...
            "Low Risk": {
                TEST_PRIMARY_RESULT_CODE: "--",
                TEST_PRIMARY_RESULT: "Low Risk",
                TEST_SECONDARY_RESULT_CODE: "--",
                TEST_SECONDARY_RESULT: "--"
            },
            "At Risk": {
                TEST_PRIMARY_RESULT_CODE: "--",
                TEST_PRIMARY_RESULT: "At Risk",
                TEST_SECONDARY_RESULT_CODE: "--",
                TEST_SECONDARY_RESULT: "--"
            }
        };
    ...
    ***********************************************************************************/
    ...
    function mapSingleNormalizedRecord(signature, hierarchy, row, year) {
        ...
        strandsAndFields.forEach(function(strandsAndField) {
            ...
            var score = row.getField("${assessmentCode}"+"-SCORE");
            var performanceCode = _util.coalesce(_util.trim(row.getField("${assessmentCode}"+"-LEVELS")), "--");
            var ClientDate = row.getField("CLIENT_DATE");
            var lexileScore = row.getField("DIBELS COMPOSITE SCORE - LEXILE");
            var lexile = null;
            if(lexileScore && strandsAndField.STRAND === "TOT" ){
                lexile = lexileScore.replace("BR", "").replace("L","");
            }

            if(score!==null && performanceCode!=='--' && ClientDate!==null){
                mappingObject.SCORE_MAPPINGS.push(
                    {
                        "TEST_NUMBER" : "${strandsAndField.TEST}" + gradeLevels[row.getField("ASSESSMENT_GRADE")] + getShortAdminPeriod(row.getField("BENCHMARK_PERIOD")) +"${strandsAndField.STRAND}",
    ...
    ***********************************************************************************/
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row, year) {
        ...
        // TEST_ADMIN_DATE
        var testDateField = _util.coalesce(row.getField("CLIENT_DATE"),("05/01/" + hierarchy.SCHOOL_YEAR.toString().substring(5,9)));
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "STUDENT_LOCAL_ID":
                    record.STUDENT_LOCAL_ID = _util.coalesce(row.getField("PRIMARY_ID_LOCAL_STUDENT_ID"));
                    break;
                ...
                case "STUDENT_VENDOR_ID":
                    record.STUDENT_VENDOR_ID = _util.coalesce(
                        row.getField("PRIMARY_ID_LOCAL_STUDENT_ID"),
                        row.getField("STATE_STUDENT_ID"),
                        row.getField("STUDENT_FIRST_NAME") + "_" +
                        row.getField("STUDENT_MIDDLE_NAME") + "_" +
    ...
    ***************************************************************************/
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord) {
        ...
        assessmentAdminKey = "${_assessmentIdentifier}_${_util.coalesce(AssessmentLoader.config.SYS_PARTITION_VALUE, AssessmentLoader.config.TENANT_CODE)}_${AssessmentLoader.config.DISTRICT_CODE}_" + getTEST_ADMIN_PERIOD(srcRecord.getField("BENCHMARK_PERIOD"));

        var DateObj = module.mapDateObject(hierarchy, srcRecord, srcRecord.getField("CLIENT_DATE"));
