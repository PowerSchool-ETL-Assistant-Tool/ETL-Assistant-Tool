var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "G3G6_RD_WR_MA";
   ...
   module.createNormalizedFile = function(file, signature) {
    ...
    var scoresToMap = [
        {TEST: "G3G6", CODE:"RD", FIELD: "READING_", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "G3G6", CODE:"WR", FIELD: "WRITING_", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "G3G6", CODE:"MA_OVR", FIELD: "MATH_", MAP_FUNCTION: mapScaledScore}
    ];
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        if (birthdate === -1 || birthdate === '' || birthdate === "-1")
        {
         birthDateObject = ''
        }
        else {
            var patterns = ["yyyyMMdd", "yyyy-MM-dd", "MM/dd/yyyy"];
            for (var i = 0; i < patterns.length; i++) {
                birthDateObject = AppUtil.getDateObjByPattern(birthdate, patterns[i]);
            if (birthDateObject && birthDateObject.YEAR && birthDateObject.MONTH && birthDateObject.DAY) {
            break;
            }
        }
        if (!birthDateObject || !birthDateObject.YEAR) {
        birthDateObject = '';
        }
        }

            // Default values
            var testDate = '05/15/' + hierarchy.SCHOOL_YEAR.substring(5, 9);  // Default date
            ...
            normalizedFileFields.forEach(function (field) {
                switch(field) {
                    ...
                    case "STUDENT_STATE_ID":
                        record.STUDENT_STATE_ID = _util.coalesce(row.getField("STUDENTOEN"),row.getField("STATE_STUDENT_ID"));
                        break;
                    case "STUDENT_VENDOR_ID":
                        record.STUDENT_VENDOR_ID = _util.coalesce(row.getField("STUDENTOEN"));
                        break;
    ...
    function mapScaledScore(row, testMetadata) {
		...
		var primaryResult = {}
		if (scaleScore >= 0.1 && scaleScore <= 0.9) {
            primaryResult = "Not Enough Evidence";
        }else if (scaleScore >= 1.1 && scaleScore <= 1.9) {
             primaryResult = "Level 1";
        }else if (scaleScore >= 2.1 && scaleScore <= 2.9) {
              primaryResult = "Level 2";
        }else if (scaleScore >= 3.1 && scaleScore <= 3.9) {
              primaryResult = "Level 3";
        }else if (scaleScore >= 4.1 && scaleScore <= 4.9) {
               primaryResult = "Level 4";
        }

        // Check for key fields and do not map score and exit if conditions met.
        if(scaleScore === null || scaleScore === undefined || _util.trim(scaleScore) === "--" || _util.trim(scaleScore) === "" || scaleScore === "-99" ) {
            return null;
        }
