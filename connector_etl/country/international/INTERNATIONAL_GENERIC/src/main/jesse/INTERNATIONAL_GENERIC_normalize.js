var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "INTERNATIONAL_GENERIC";
    ...
    var scoresToMap = [
        {TEST: "INT_GENERIC", MAP_FUNCTION: mapScaledScore}
    ];
...
//        var birthDateObject = AppUtil.getDateObjByPattern(row.getField("STUDENT_BIRTHDATE"), "yyyy-MM-dd");
        ...
        try {
            if (signature.getFormat().getFormatName() === "GENERIC_SIG_15_COLS") {
        ...
        if(row.getField("REPORTING_PERIOD")){
            var period = row.getField("REPORTING_PERIOD").toUpperCase().replace("-","");
        }
        var testDate1;
        var tt = row.getField("TEST_ADMIN_DATE");

        if (tt!== null){
            testDate1 = AppUtil.getMonthFollowedByDay(row.getField("TEST_ADMIN_DATE")).STANDARD_DATE;
        }else{
            if(period === "MIDYEAR") {
                testDate1 = '01/15/' + hierarchy.SCHOOL_YEAR.substring(5, 9);
            } else if(period === "SPRING") {
                testDate1 = '05/15/' + hierarchy.SCHOOL_YEAR.substring(5, 9);
            }else if(period === "FALL") {
                testDate1 = '09/15/' + hierarchy.SCHOOL_YEAR.substring(0, 4);
            }else {
                testDate1 = '05/15/' + hierarchy.SCHOOL_YEAR.substring(5, 9);
            }
        }
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = testDate1;
                    break;
                //Student Information
                case "STUDENT_VENDOR_ID":
                    record.STUDENT_VENDOR_ID = row.getField("VENDOR_STUDENT_ID");
                    break;
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = _util.coalesce(row.getField("STUDENT_STATE_ID"), row.getField("VENDOR_STUDENT_ID"));
                    break;
                case "STUDENT_LOCAL_ID":
                    record.STUDENT_LOCAL_ID = row.getField("VENDOR_STUDENT_ID");
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("VENDOR_SCHOOL_ID"),row.getField("SCHOOL_NAME"));
                    break;
                 case "SCHOOL_NAME":
                      record.SCHOOL_NAME = _util.coalesce(row.getField("SCHOOL_NAME"));
                      break;
                case "SCHOOL_STATE_ID":
                     record.SCHOOL_STATE_ID = _util.coalesce(row.getField("SCHOOL_STATE_ID"),row.getField("VENDOR_SCHOOL_ID"));
                     break;
                case "SCHOOL_LOCAL_ID":
                     record.SCHOOL_LOCAL_ID = _util.coalesce(row.getField("VENDOR_SCHOOL_ID"));
                     break;
    ...
    function mapScaledScore(row, testMetadata) {
        var score = {};
        var testNumber = testMetadata.TEST + "_" + row.getField("TEST_ID");
        var scaleScore = row.getField("TEST_SCALED_SCORE");
        var itemsEarned = row.getField("TEST_ITEMS_EARNED");
        var itemsPossible = row.getField("TEST_ITEMS_POSSIBLE");
        var level = row.getField("TEST_PRIMARY_RESULT");
    	...
    	score["TEST_ITEMS_ATTEMPTED"] = parseInt(itemsEarned);
