var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "EMSAT";
    ...
    var scoresToMap = [
        {TEST: "EMSAT", CODE:"OVR", FIELD: "OVERALL", MAP_FUNCTION: mapOverallScore}
        ,{TEST: "EMSAT", CODE:"RD", FIELD: "READING", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "EMSAT", CODE:"WR", FIELD: "WRITING", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "EMSAT", CODE:"MA", FIELD: "MATH", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "EMSAT", CODE:"BIO", FIELD: "BIOLOGY", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "EMSAT", CODE:"CHEM", FIELD: "CHEMISTRY", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "EMSAT", CODE:"AB", FIELD: "ARABIC", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "EMSAT", CODE:"PY", FIELD: "PHYSICS", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "EMSAT", CODE:"ENG", FIELD: "ENGLISH", MAP_FUNCTION: mapScaledScore}
    ];
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        if (studentname !== null && studentname !== undefined && studentname !== "" && studentname.toString().indexOf(",")>-1){
           firstName = studentname.split(",")[1];
           lastName = studentname.split(",")[0];
        }
        if (row.getField("TEST_ADMIN_DATE")) {
            var testAdminDate = row.getField("TEST_ADMIN_DATE");
            if (testAdminDate.substring(2, 3) === "/") {
                var assessmentDateObject = AppUtil.getDateObjByPattern(testAdminDate, "MM/dd/yyyy");
            } else if (testAdminDate.indexOf("-") > -1) {
                var assessmentDateObject = AppUtil.getDateObjByPattern(testAdminDate, "dd-MMMM-yyyy");
            }
        }
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "STUDENT_VENDOR_ID":
                    ...
                    break;
                    //The student name was mapped only to first name because we were unsure which part will be first name or last name
    ...
    function mapOverallScore(row, testMetadata) {
        var score = {};
        var sub = subjectDecode[row.getField("TEST_SUBJECT")];
        var testNumber = testMetadata.TEST + "_" + testMetadata.CODE + "_" + sub;
        var scaleScore = row.getField(testMetadata.FIELD + "_RESULTS");
        var testShift = row.getField("TEST_SHIFT");
        // Check for key fields and do not map score and exit if conditions met.
        if(scaleScore === null || scaleScore === undefined || _util.trim(scaleScore) === "--" || _util.trim(scaleScore) === "" || scaleScore === "Pending Payment") {
            return null;
        }
        ...
        score["TEST_SCORE_ATTRIBUTES"] = testShift;
        return score;
    }

//When the connector was created there was no data provided, So just have created these functions for future accommodations
//Please feel free to alter this based upon the requirements
//Reference UIHN-73910
    function mapScaledScore(row, testMetadata) {
        var score = {};
        var testNumber = testMetadata.TEST + testMetadata.CODE;
        var scaleScore = _util.coalesce(row.getField("ENGLISH_" + testMetadata.FIELD), row.getField("ARABIC_" + testMetadata.FIELD));
        var performance = row.getField("ENGLISH_BAND");
        // Check for key fields and do not map score and exit if conditions met.
        ...
        function formatDate(dateString) {
            if (dateString !== null && dateString !== "" && dateString !== undefined) {
                if (dateString.substring(2, 3) === "/") {
                    ...
                    // dd-MMMM-yyyy
                    return AppUtil.getDateObjByPattern(dateString, "dd-MMMM-yyyy").STANDARD_DATE;
                }
            }
            return dateString;
        }
