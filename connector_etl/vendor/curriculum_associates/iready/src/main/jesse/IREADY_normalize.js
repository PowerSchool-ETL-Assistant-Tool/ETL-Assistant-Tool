var AppNormalize = (function () {
    ...
    var _assessmentIdentifier = "IREADY";
    ...
    function mapNormalizedRecords(signature, hierarchy, rows) {
        ...
        for (var i = 0; i <= 5; i++) {
            if(row.getField("COMPLETION_DATE_" + i)!==undefined){//Testing if this speeds up the process for tests with only latest
        ...
        normalizedFileFields.forEach(function (field) {
            switch (field) {
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("SCHOOL"), row.getField("SCHOOL_ID"), row.getField("SCHOOL_STATE_ID"),"DISTRICT");
                    break;
                case "SCHOOL_NAME":
    ...
    function mapAdditionalAdminFields(row) {
        var admin = {};
        var code = "--";
        var startDateField = row.getField("START_DATE_0");
        ...
        if (null != rushFlag && "" != rushFlag) {
            ...
            if (startDateField) {
            var newFormat = getDateFormat(startDateField);
            var startDate = AppUtil.getDateObjByPattern(startDateField, newFormat).STANDARD_DATE;
            admin.TEST_EXEMPTION_DESC = startDate;
            admin.TEST_EXEMPTION_CODE = "Start Date";
        }
        return admin;
    }
    ...
    function mapScaledScore(row, testMetadata, attempt,signature) {
        ...
        } else if( testMetadata.SUBJECT == 'RD_SPA' && (signature.format.formatName === "iReady_2223_spanish" || signature.format.formatName === "iReady_23_24_Spanish_ELA")){
            subject = "RD_SPA";
        } else if (testMetadata.SUBJECT === 'OVERALL' && (signature.format.formatName === "iReady_2223_spanish" || signature.format.formatName === "iReady_23_24_Spanish_ELA")) {
            subject = "ELA_SPA";
            isELASPA = true;
            isELA = true;
        }else if(isELA == false) {
        ...
        var testNumber = 'IREADY_' + subject + '_' + testMetadata.TEST_NUMBER_SUFFIX;
        ...
        if (((scaleScore === null || scaleScore === undefined || _util.trim(scaleScore) === "" || _util.trim(scaleScore) === "NULL") &&
            placementScore !== "Tested Out" &&
            placementScore !== "Surpassed Level" // UIHN-74123
          ) &&
          (percentScore === null || percentScore === undefined || _util.trim(percentScore) === "") &&
          (relativePlacementScore === null || relativePlacementScore === undefined || _util.trim(relativePlacementScore) === "") &&
          isELASPA === false
        ) {
          return null;
        }
        ...
        var lower = '';
        var upper = '';
        if(testMetadata.SUBJECT == "OVERALL" || testMetadata.SUBJECT == "ELA_SPA"  && subject == "ELA" && row.getField("LEXILE_MEASURE_0") !== null) {
            ...
            if (LexileRange !== null && LexileRange !== undefined && LexileRange !== "" && LexileRange.contains('-')) {
                lower = LexileRange.split("-")[0];
                upper = LexileRange.split("-")[1];

                // Cleanup before calling replace
                lower = lower ? lower.replace("L", "").replace("BR", "").trim() : null;
                upper = upper ? upper.replace("L", "").replace("BR", "").trim() : null;
            } else {
                var rangeField = row.getField("LEXILE_RANGE_0");
                lower = rangeField ? rangeField.replace("L", "").replace("BR", "").trim() : null;
            }
        }
        if (testMetadata.SUBJECT == "OVERALL" && (subject == "MATH" || subject == "MATH_SPA") && row.getField("QUANTILE_MEASURE_0") !== null) {
            ...
            if (QuantileRange !== null && QuantileRange !== undefined && QuantileRange !== "" && QuantileRange.contains('-')) {
                lower = QuantileRange.split("-")[0];
                upper = QuantileRange.split("-")[1];

                // Cleanup before calling replace
                lower = lower ? lower.replace("Q", "").replace("EM", "").trim() : null;
                upper = upper ? upper.replace("Q", "").replace("EM", "").trim() : null;
            } else {
                var quantileField = row.getField("QUANTILE_RANGE_0");
                lower = quantileField ? quantileField.replace("Q", "").replace("EM", "").trim() : null;
            }
        }
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord) {
        ...
        if ((JSON.stringify(srcRecord)).indexOf("LEXILE_MEASURE") > -1 || (JSON.stringify(srcRecord)).indexOf("PHONICS_LESSONS_COMPLETED") >-1 || (JSON.stringify(srcRecord)).indexOf("COMPREHENSION_LITERATURE_%_CORRECT") >-1 ) {
            subject = "ELA";
            isELA = true;
        }
        ...
        //ACADEMIC_YEAR format changed from yearYYYY-YYYY to YYYY-YYYY on 12/13/2019.  Added code to keep sortkey the same across file versions
        //removed 6/15/2020 as YYYY-YYYY is more preferable. Data will report as it reports in the field in the file
        //var academicYear = trim(row.getField('ACADEMIC_YEAR'));
        //if(academicYear.substring(0,4) !== "year") {
        //academicYear = "year" + trim(row.getField('ACADEMIC_YEAR'));
        //}
        ...
        var sigName= signature.getFormat().getFormatName();
        if(sigName === "iReady_2122_ELA" || sigName === "iReady_2122_MA" || sigName === "iReady_MATH_2223") {
           ...
           assessmentAdminKey = assessmentAdminKey + "_" + studentId;
        }
        ...
        //Add date manually to conform
          // START_DATE
          if(signature.format.formatName === "iReadyDiagnostic_ELA_2023" || signature.format.formatName === "IREADY_MA_50_COLS" || signature.format.formatName === "IREADY_2022_23MATH"){
            if (srcRecord.getField("START_DATE_0") !== "NULL") {
                if (srcRecord.getField("START_DATE_0") !== null && srcRecord.getField("START_DATE_0") !== undefined) {
                    var newFormat = getDateFormat(srcRecord.getField("START_DATE_0"));
                    var startDate = AppUtil.getDateObjByPattern(srcRecord.getField("START_DATE_0"), newFormat).STANDARD_DATE;

                } else {
                    var startDate = null;
                }
                assessmentAdminKey = assessmentAdminKey + "_" + startDate + "_" + srcRecord.getField("TIME_ON_TASK_MIN_0");
            } else {
                var startDate = null;
            }
            }
        ...
        //TODO consider re-addition of RUSH_FLAG_0 and READING_DIFFICULTY_INDICATOR based on other signatures.
