var AppNormalize = (function () {
    ...
    const _assessmentIdentifier = "BRIGANCE";
    ...
    const SCORES_TO_MAP = [
        { TEST: 'BRI', CODE: 'TOTAL', fn: collectScores },
        { TEST: 'BRI', CODE: 'ACAD_COG', fn: collectScores },
        { TEST: 'BRI', CODE: 'LANG_DEV', fn: collectScores },
        { TEST: 'BRI', CODE: 'PHYS_DEV', fn: collectScores },
        { TEST: 'BRI', CODE: 'ADAPTIVE_BEHAVIOR', fn: collectScores },
        { TEST: 'BRI', CODE: 'SELF_HELP_SKILLS', fn: collectScores },
        { TEST: 'BRI', CODE: 'SOC_EMO_SKILLS', fn: collectScores }
    ];
    ...
    const ASSESSMENT_LANGUAGE_DECODE = {
        "English": "EN",
        "Spanish": "SP",
        "Japanese": "JA",
        "English and Spanish": "ES",
        "English and Japanese": "EJ",
        "Mandarin (Putonghua)": "MP",
        "French": "FR",
        "Other": "OT",
        "Chinese": "CN",
        "": "EN",
    ...
    var performanceDecode = {
        "Below Delays Cutoff" : "BDC",
        "Within Normal Limits" : "WNL",
        "Above Gifted Cutoff" : "AGC",
        "Below Average" : "BAVG",
        "Average" : "AVG",
        "Above Average" : "AAVG"
    };
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        let birthDateObject = AppUtil.getDateObjByPattern(row.getField("DATE_OF_BIRTH"), "MM/dd/yy");
        ...
        module.normalizedFileFields.forEach((field) => {
            switch (field) {
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = AppUtil.getDateObjByPattern(row.getField("SCREEN_DATE"), "MM/dd/yy").STANDARD_DATE;
                    break;

                //Student Information
                case "STUDENT_LOCAL_ID":
                    record.STUDENT_LOCAL_ID = _util.coalesce(
                        row.getField("LOCAL_ID"),row.getField("STUDENT_ID")
                    );
                    break;
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = _util.coalesce(
                        row.getField("SSID"),row.getField("STUDENT_ID")
                    );
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(
                        row.getField("SCHOOL"),row.getField("SCHOOL/CLASS_ID"),row.getField("SCHOOL/CLASS_NAME"),
                        "DISTRICT"
                    );
                    break;
    ...
    function collectScores(row, testMetadata) {
        let compositeScore = _util.trim(row.getField(testMetadata.CODE + "_COMPOSITE_SCORE"));

        if ([null, undefined, '--'].some(falsy => falsy === compositeScore)) {
            return null;
        }
        ...
        let percentile = _util.trim(row.getField(testMetadata.CODE + "_PERCENTILE"));
        if(percentile !== null && percentile !== undefined && _util.trim(percentile) !=="--" && _util.trim(percentile) !== ""){
            percentile = percentile.replace(/<|>/g,"");
        }

        let score = {
            TEST_NUMBER: generateTestNumber(row, testMetadata),
            TEST_SCORE_TEXT: compositeScore
        };
        compositeScore = compositeScore.replace(/<|>/g,"");
        if (!isNaN(compositeScore)) {
            score.TEST_SCALED_SCORE = compositeScore;
            score.TEST_SCORE_VALUE = compositeScore;
        }

        score.TEST_RAW_SCORE = rawScore;
        score.TEST_PERCENTILE_SCORE = percentile;

        var performanceLevel = row.getField(testMetadata.CODE + "_PERFORMANCE_SUMMARY");
        if(performanceLevel !== null, performanceLevel !== undefined, performanceLevel !=="--"){
            if (performanceLevel.indexOf("_")>-1) {
                let performanceInfo = (() => {
                let performanceSummary = _util.trim(row.getField(testMetadata.CODE + "_PERFORMANCE_SUMMARY"));
                if (performanceSummary) {
                    return performanceSummary.split("_");
                }
                return null;
                })();

                if (performanceInfo && performanceInfo[0] && performanceInfo[1]) {
                    score.TEST_PRIMARY_RESULT = performanceInfo[1];
                    score.TEST_PRIMARY_RESULT_CODE = performanceInfo[0];
                }
            }
            else {
                score.TEST_PRIMARY_RESULT = performanceLevel;
                score.TEST_PRIMARY_RESULT_CODE = performanceDecode[performanceLevel];
            }
        }
    ...
    function generateAssessmentAdminKey(signature, hierarchy, srcRecord) {
        ...
        var date = srcRecord.getField("SCREEN_DATE");
    ...
    function generateTestNumber(row, tmd) {
        return tmd.TEST + '_' + tmd.CODE + '_' + ASSESSMENT_LANGUAGE_DECODE[_util.trim(row.getField("ASSESSMENT_LANGUAGE"))];;
    }
    return module;
}());