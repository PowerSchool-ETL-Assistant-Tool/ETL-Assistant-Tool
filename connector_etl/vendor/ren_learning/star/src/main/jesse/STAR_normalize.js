var AppNormalize = (function () {
    ...
    function mapTOTAL(row, testMetadata) {
        try {
            ...
            */
            ...
            score["TEST_PRIMARY_RESULT"] = _util.coalesce(row.getField("SCREENING_CATEGORY"), row.getField("SCREENING_CATEGORY_GROUP_ADJUSTMENT"), row.getField("SCREENINGCATEGORYGROUPADJUSTMENT"), row.getField("MASTERY_LEVEL"));
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        var birthDate = null;
        if ((signature.getFormat().getFormatName() === 'REN_LEARNING_LATEST_SR' || signature.getFormat().getFormatName() === 'REN_LEARNING_LATEST_SM') && row.getField("STUDENT_BIRTHDATE") !== null) {
            if(row.getField("STUDENT_BIRTHDATE").length === 5 && row.getField("STUDENT_BIRTHDATE").substring(0,2) > 31) {
                birthDate = null ;
            }
            else {
            birthDate = row.getField("STUDENT_BIRTHDATE"), "MM/dd/yyyy";
            }
    ...
    function getTEST_NUMBER(row, scoreType) {
        ...
        var grade = decodeGrade(_util.coalesce(row.getField("STUDENT_GRADE_CODE"), row.getField("GRADE"), row.getField("CURRENT_GRADE"), row.getField("CURRENTGRADE"), row.getField("ASSESSMENT_GRADE")));
