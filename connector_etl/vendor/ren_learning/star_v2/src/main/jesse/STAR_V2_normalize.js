var AppNormalize = (function() {
    ...
    var perfLevels = {
        ...
        //new decodes StateBenchmarkCategoryName?
    };
    ...
    function mapTOTAL(row, testMetadata, signature) {
        try {
           ...
           // var perfObject = decodePerfLevel(_util.coalesce(row.getField("RENAISSANCEBENCHMARKCATEGORYNAME")));
            ...
            score["TEST_CUSTOM_RESULT_CODE"] = _util.coalesce(row.getField("STATEBENCHMARKCATEGORYNAME"),row.getField("STATEBENCHMARKCATEGORYADJUSTMENT"));
            rawGE = null;
    ...
    function mapBenchmarkScore(row, testMetadata) {
        try {
            ...
            //Upper and lower bounds for these categories are based on the master scaled score
            var scaleScore = _util.coalesce(row.getField("UNIVERSALSCORE"),row.getField("SCALED_SCORE"),row.getField("SCALEDSCORE"), row.getField("ENTERPRISE_SCORE"), row.getField("AVERAGE_MASTERY"));
     ...
     */
     function mapAdditionalAdminFields(row) {
         ...
         admin.TEST_INTERVENTION_DESC_4 = status;
    ...
    function determineFileSubject(format,normalizedFile) {
        ...
        if (type === 'ALL360_V2' && fileName.indexOf('Star_Early_Literacy')> -1) {
           ...
           subject = 'MA';
        }else if (fileName.indexOf('SEL_V2')> -1){
           subject = 'LIT';
        }else if (fileName.indexOf('SEL_v2')> -1){
           subject = 'LIT';
        } else if (fileName.indexOf('SELS') > -1) {
            subject = 'SPLIT';
        } else if (fileName.indexOf('SRS') > -1) {
            subject = 'SPRD';
        }else {
            ...
            if(tad.length > 10) {
                ...
                if (dateVal2.indexOf("/") > -1){
                    ...
                    if(dateVal2.length<10){
                        sd = AppUtil.getDateObjByPattern(dateVal2, "M/d/yy").STANDARD_DATE;
                    }
                    else {
...
module.mapDateObject = function (hierarchy, row, dateStr) {
        ...
        try {
            testDateObject = AppUtil.getDateObjByPattern(dateStr, "M/d/yy");
        } catch(exception){
