var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "Dreambox";
     ...
     ***************************************************************************/
    var scoresToMap = [
        {TEST: "DREAMBOX", CODE:"DREAMBOX",MAP_FUNCTION: mapScaledScore}
    ];
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        var studentID = _util.coalesce(row.getField("STUDENT_ID"));
        var replaceDBLID = studentID.replace("dblx_","");
        replaceDBLID = replaceDBLID.replace("dbl_","");
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = AppUtil.getDateObjByPattern(row.getField("END_DATE"), "M/d/yyyy").STANDARD_DATE;
                    break;
                ...
                case "STUDENT_LOCAL_ID":
                    record.STUDENT_LOCAL_ID = replaceDBLID;
                    break;
    ...
    function mapScaledScore(row, testMetadata) {
        var score = {};
        var testNumber = testMetadata.CODE;
        var mins = row.getField("MINS");
		var ext_mins = row.getField("EXT_MINS");
		var lessons_complted = row.getField("LESSONS_COMPLETED");
		var lessons_passed = row.getField("LESSONS_PASSED");
		var growth = row.getField("GROWTH");

        // Check for key fields and do not map score and exit if conditions met.
        if(lessons_complted === null || lessons_complted === undefined || _util.trim(lessons_complted) === "--" || _util.trim(lessons_complted) === "") {
            return null;
        }

    	// Set score fields
    	score["TEST_NUMBER"] = testNumber;
    	score["TEST_SCORE_TEXT"] = lessons_passed;
    	score["TEST_SCORE_VALUE"] = lessons_passed;
    	score["TEST_SCALED_SCORE"] = lessons_passed;
    	score["TEST_ITEMS_POSSIBLE"] = lessons_complted;
    	score["TEST_ITEMS_ATTEMPTED"] = lessons_passed;
    	score["TEST_PRIMARY_RESULT"] = mins;
    	score["TEST_SECONDARY_RESULT"] = ext_mins;
