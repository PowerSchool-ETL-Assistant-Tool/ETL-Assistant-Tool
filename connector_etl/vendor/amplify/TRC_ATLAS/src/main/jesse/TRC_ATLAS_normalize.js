var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "TRC_ATLAS";
    ...
    var scoresToMap = [
        {TEST: "", CODE:"", FIELD: "", MAP_FUNCTION: mapOverallScore}
    ];
    ...
    function mapOverallScore(row, testMetadata) {
        try {
            ...
            if(fileName.indexOf("trc") >-1 || fileName.indexOf("TRC") >-1){
                testNumber = "TRC" + "_" + row.getField("BENCHMARK_PERIOD");
            }else if(fileName.indexOf("atlas") >-1 || fileName.indexOf("ATLAS") >-1){
                if(row.getField("ASSESSMENT_EDITION").toString().indexOf("Espanol")>-1){
                    testNumber = "ATLAS_ESPANOL" + "_" + row.getField("BENCHMARK_PERIOD");
                }
                else{
                    testNumber = "ATLAS_ENG" + "_" + row.getField("BENCHMARK_PERIOD");
                }
            }
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_REPORTING_DATE":
                    record.TEST_REPORTING_DATE = "01/01/" + row.getField("SCHOOL_YEAR").substring(5, 9);
                    break;
                ...
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = _util.coalesce(row.getField("STUDENT_PRIMARY_ID"));
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = row.getField("SCHOOL_NAME");
                    break;
                default:
