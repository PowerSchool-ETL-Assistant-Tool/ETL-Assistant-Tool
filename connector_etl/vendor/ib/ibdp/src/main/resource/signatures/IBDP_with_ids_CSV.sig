{
  "encoding": "UTF-8",
  "format": {
    "@type": "Delimited",
    "formatName": "IBDP_with_ids_CSV",
    "quoteDelim": "\"",
    "fieldDelim": ",",
    "hasHeader": true,
    "naturalKey": ["STUDENT_ID", "SUBJECT", "YEAR","LEVEL", "REGISTRATION_NUMBER"],
    "fields": [{
      "physicalName": "Year",
      "otherPossibleNames": [],
      "logicalName": "YEAR",
      "formatMask": null,
      "isFieldRequired": "Y",
      "canBeNull": "Y",
      "fieldOffset": {
        "offsetStart": 0,
        "offsetEnd": 0,
        "columnNum": 1
      },
      "fieldLength": 100,
      "expectedValues": [],
      "dataTypes": "STRING",
      "scale": 0.0
    },
      {
        "physicalName": "Month",
        "otherPossibleNames": [],
        "logicalName": "MONTH",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 2
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "School",
        "otherPossibleNames": [],
        "logicalName": "SCHOOL_ID",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "N",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 3
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Registration number",
        "otherPossibleNames": [],
        "logicalName": "REGISTRATION_NUMBER",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 4
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Personal code",
        "otherPossibleNames": [],
        "logicalName": "PERSONAL_CODE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 5
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Name",
        "otherPossibleNames": [],
        "logicalName": "STUDENT_NAME",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 6
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Local ID",
        "otherPossibleNames": ["Student ID"],
        "logicalName": "STUDENT_ID",
        "formatMask": null,
        "isFieldRequired": "N",
        "canBeNull": "N",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 7
        },
        "fieldLength": 30,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Category",
        "otherPossibleNames": [],
        "logicalName": "CATEGORY",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 8
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Subject",
        "otherPossibleNames": [],
        "logicalName": "SUBJECT",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 9
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Level",
        "otherPossibleNames": [],
        "logicalName": "LEVEL",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 10
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Language",
        "otherPossibleNames": [],
        "logicalName": "LANGUAGE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 11
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Predicted grade",
        "otherPossibleNames": [],
        "logicalName": "PREDICTED_GRADE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 12
        },
        "fieldLength": 100,
        "expectedValues": ["", " ", "A", "B", "C", "D", "E", "F", "N", "1", "2", "3", "4", "5", "6", "7"],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Grade",
        "otherPossibleNames": [],
        "logicalName": "GRADE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "N",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 13
        },
        "fieldLength": 30,
        "expectedValues": ["", " ", "A", "B", "C", "D", "E", "F", "N", "1", "2", "3", "4", "5", "6", "7"],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "EE/TOK points:",
        "otherPossibleNames": [],
        "logicalName": "EE_TOK_POINTS",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 14
        },
        "fieldLength": 100,
        "expectedValues": ["", " ", "1", "2", "0", "N"],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Total points:",
        "otherPossibleNames": [],
        "logicalName": "TOTAL_POINTS",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 15
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Result:",
        "otherPossibleNames": [],
        "logicalName": "RESULT",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 16
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "Diploma requirements code:",
        "otherPossibleNames": [],
        "logicalName": "DIPLOMA_REQUIREMENTS_CODE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 17
        },
        "fieldLength": 500,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      }
    ]
  },
  "fileExtensions": [".csv"]
}