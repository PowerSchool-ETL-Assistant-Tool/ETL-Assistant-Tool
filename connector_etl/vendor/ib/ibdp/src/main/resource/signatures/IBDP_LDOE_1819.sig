{
  "encoding": "UTF-8",
  "format": {
    "@type": "Delimited",
    "formatName": "IBDP_LDOE_1819",
    "quoteDelim": "\"",
    "fieldDelim": ",",
    "hasHeader": true,
    "naturalKey": ["STUDENT_ID", "SUBJECT", "YEAR"],
    "fields": [{
      "physicalName": "YEAR",
      "otherPossibleNames": [],
      "logicalName": "YEAR",
      "formatMask": null,
      "isFieldRequired": "Y",
      "canBeNull": "Y",
      "fieldOffset": {
        "offsetStart": 0,
        "offsetEnd": 0,
        "columnNum": 1
      },
      "fieldLength": 100,
      "expectedValues": ["2019"],
      "dataTypes": "STRING",
      "scale": 0.0
    },
      {
        "physicalName": "MONTH",
        "otherPossibleNames": [],
        "logicalName": "MONTH",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 2
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "PROGRAMME",
        "otherPossibleNames": [],
        "logicalName": "PROGRAMME",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 3
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "REGIONAL_OFFICE",
        "otherPossibleNames": [],
        "logicalName": "REGIONAL_OFFICE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 4
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "COUNTRY",
        "otherPossibleNames": [],
        "logicalName": "COUNTRY",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 5
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "STATE",
        "otherPossibleNames": [],
        "logicalName": "STATE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 6
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "SCHOOL_CODE",
        "otherPossibleNames": [],
        "logicalName": "SCHOOL_ID",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "N",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 7
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "SCHOOL_NAME",
        "otherPossibleNames": [],
        "logicalName": "SCHOOL_NAME",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 8
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "LEGAL_STATUS",
        "otherPossibleNames": [],
        "logicalName": "LEGAL_STATUS",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 9
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "CANDIDATE",
        "otherPossibleNames": [],
        "logicalName": "CANDIDATE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 10
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "FIRST NAME",
        "otherPossibleNames": [],
        "logicalName": "FIRST_NAME",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 11
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "LAST_NAME",
        "otherPossibleNames": [],
        "logicalName": "LAST_NAME",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 12
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "DOB",
        "otherPossibleNames": [],
        "logicalName": "DATE_OF_BIRTH",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 13
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "GENDER",
        "otherPossibleNames": [],
        "logicalName": "GENDER",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 14
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "CATEGORY",
        "otherPossibleNames": [],
        "logicalName": "CATEGORY",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 15
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "GROUP NUMBER",
        "otherPossibleNames": [],
        "logicalName": "GROUP NUMBER",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 16
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "SUBJECT",
        "otherPossibleNames": [],
        "logicalName": "SUBJECT",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 17
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "LEVEL",
        "otherPossibleNames": [],
        "logicalName": "LEVEL",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 18
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "LANGUAGE",
        "otherPossibleNames": [],
        "logicalName": "LANGUAGE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 19
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "PREDICTED_GRADE",
        "otherPossibleNames": [],
        "logicalName": "PREDICTED_GRADE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 20
        },
        "fieldLength": 100,
        "expectedValues": ["", " ", "A", "B", "C", "D", "E", "F", "N", "1", "2", "3", "4", "5", "6", "7"],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "SUBJECT_GRADE",
        "otherPossibleNames": [],
        "logicalName": "GRADE",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "N",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 21
        },
        "fieldLength": 30,
        "expectedValues": ["", " ", "A", "B", "C", "D", "E", "F", "N", "1", "2", "3", "4", "5", "6", "7"],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "EE_TOK_BONUS_POINTS",
        "otherPossibleNames": [],
        "logicalName": "EE_TOK_POINTS",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 22
        },
        "fieldLength": 100,
        "expectedValues": ["", " ", "1", "2", "3", "0", "N"],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "TOTAL_POINTS",
        "otherPossibleNames": [],
        "logicalName": "TOTAL_POINTS",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 23
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "RESULT_CODE",
        "otherPossibleNames": [],
        "logicalName": "RESULT",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "Y",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 24
        },
        "fieldLength": 100,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      },
      {
        "physicalName": "LASID",
        "otherPossibleNames": [],
        "logicalName": "STUDENT_ID",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "N",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 25
        },
        "fieldLength": 30,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      }
    ,
      {
        "physicalName": "LEA",
        "otherPossibleNames": [],
        "logicalName": "SCHOOL_LEA",
        "formatMask": null,
        "isFieldRequired": "Y",
        "canBeNull": "N",
        "fieldOffset": {
          "offsetStart": 0,
          "offsetEnd": 0,
          "columnNum": 26
        },
        "fieldLength": 30,
        "expectedValues": [],
        "dataTypes": "STRING",
        "scale": 0.0
      }
    ]
  },
  "fileExtensions": [".csv"]
}