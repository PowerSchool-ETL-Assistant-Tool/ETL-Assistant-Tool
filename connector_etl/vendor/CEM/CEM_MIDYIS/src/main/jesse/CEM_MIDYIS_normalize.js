var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "CEM_MIDYIS";
    ...
    var scoresToMap = [
        {TEST: "CEM_MIDYIS_", CODE:"", FIELD: "TEST_", MAP_FUNCTION: mapScaledScore}
    ];
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_ADMIN_DATE":
                       record.TEST_ADMIN_DATE = _util.coalesce(AppUtil.getDateObjByPattern(row.getField("TEST_ADMIN_DATE"), "dd/MM/yyyy").STANDARD_DATE,'09/07/' + hierarchy.SCHOOL_YEAR.substring(0, 4));
                    break;
                ...
                case "STUDENT_STATE_ID":
                    record.STUDENT_STATE_ID = _util.coalesce(row.getField("STUDENT_VENDOR_ID"));
                    break;
                case "STUDENT_VENDOR_ID":
                    record.STUDENT_VENDOR_ID = _util.coalesce(row.getField("STUDENT_VENDOR_ID"));
                    break;
                ...
                case "STUDENT_GRADE_CODE":
                    record.STUDENT_GRADE_CODE = gradeDecode[row.getField('TEST_STUDENT_GRADE')];
                    break;
    ...
    function mapAdditionalAdminFields(row) {
        ...
        var admin = {};
        var teacherName = row.getField("TEST_TEACHER");
        admin.TEST_TEACHER = teacherName;
        return admin;
    }
    ...
    function mapScaledScore(row, testMetadata) {
        ...
        var standard = standardDecode[row.getField("STANDARDISATION")];
        var sub = subDecode[row.getField("SUBJECT")];

        var testNumber = testMetadata.TEST + standard + "_" + sub;//CEM_MIDYIS_NAT_OVR
        var scaleScore = row.getField(testMetadata.FIELD + "SCALED_SCORE");

		var primaryResult = row.getField(testMetadata.FIELD + "PRIMARY_RESULT");

        var percentile = row.getField(testMetadata.FIELD + "PERCENTILE_SCORE");
        var stanScore = row.getField(testMetadata.FIELD + "STANINE_SCORE");
        ...
        score["TEST_STANINE_SCORE"] = stanScore;
