var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "AIMSWEBPLUS";
    ...
    var scoresToMap = [
        {TEST: "AIMS", CODE:"GRADE", FIELD: "MEASURE", MAP_FUNCTION: mapScaledScore}
        ,{TEST: "AIMS", CODE:"FALL_WIN", FIELD: "FALLWIN", MAP_FUNCTION: mapPeriodScore}
        ,{TEST: "AIMS", CODE:"WIN_SPR", FIELD: "WINSPR", MAP_FUNCTION: mapPeriodScore}
        ,{TEST: "AIMS", CODE:"FALL_SPR", FIELD: "FALLSPR", MAP_FUNCTION: mapPeriodScore}
        ,{TEST: "AIMS", CODE:"FALL_SUM", FIELD: "FALLSUM", MAP_FUNCTION: mapPeriodScore}
        ,{TEST: "AIMS", CODE:"WIN_SUM", FIELD: "WINSUM", MAP_FUNCTION: mapPeriodScore}
        ,{TEST: "AIMS", CODE:"SPR_SUM", FIELD: "SPRSUM", MAP_FUNCTION: mapPeriodScore}
        ,{TEST: "AIMS", FIELD: "THETA", MAP_FUNCTION: mapThetaScore}
        ,{TEST: "AIMS",  FIELD: "OVR", MAP_FUNCTION: mapSubjectsScores}
        ,{TEST: "AIMS",  FIELD: "CLAIM1",  MAP_FUNCTION: mapClaimScores}
        ,{TEST: "AIMS",  FIELD: "CLAIM2",  MAP_FUNCTION: mapClaimScores}
        ,{TEST: "AIMS",  FIELD: "CLAIM3",  MAP_FUNCTION: mapClaimScores}
        ,{TEST: "AIMS",  FIELD: "CLAIM4",  MAP_FUNCTION: mapClaimScores}
        ,{TEST: "AIMS",  FIELD: "CLAIM5",  MAP_FUNCTION: mapClaimScores}
        ,{TEST: "AIMS",  FIELD: "CLAIM6",  MAP_FUNCTION: mapClaimScores}
        ,{TEST: "AIMS",  FIELD: "CLAIM7",  MAP_FUNCTION: mapClaimScores}
        ,{TEST: "AIMS",  FIELD: "CLAIM8",  MAP_FUNCTION: mapClaimScores}
    ];
    ...
    var perfDecode = {
            ...
            "2"  :   "Needs Support",
            "3"   :  "On-Grade",
            "4"   :  "Mastery - Grade Level",
            "5"   : "Mastery and Above Grade Level",
           "" : null,
           null : null
    };
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        //Some files have a combined name
        var studentFullName = row.getField("STUDENT_FULL_NAME");
        var studentFirstName, studentLastName;
        if(studentFullName && studentFullName.length > -1){
            studentFirstName = studentFullName.toString().split(" ")[0];
            studentLastName = studentFullName.toString().split(" ")[1];
        }
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "STUDENT_GRADE_CODE":
                    record.STUDENT_GRADE_CODE = gradeDecode[_util.coalesce(row.getField('STUDENT_GRADE'),row.getField('GRADE'))];
                    break;
...
//        admin.TEST_INTERVENTION_DESC = row.getField("INTERVENTION_CODE");
    ...
    function mapPeriodScore(row, testMetadata, signature) {
       ...
       // var testNumber = testMetadata.TEST + "_" + testMetadata.CODE;
    	...
    	if((roi === null || roi === undefined || _util.trim(roi) === "--" || _util.trim(roi) === "") && signature.getFormat().getFormatName() !== "PEARSON_AIMS_SUMMER_SGP"){
            //Summer SGP only has SGP, no ROI, so needs to skip this check
            return null;
        }
    ...
    function mapSubjectsScores(row, testMetadata, siganture) {
        var score = {};
    ...
    function mapThetaScore(row, testMetadata, signature){
        var score = {};
        var subject = row.getField("SUBJECT");
        if(subject !== null){
            if(subject.indexOf("ELA") > -1){
        ...
        score["TEST_SCALED_SCORE"] = testThetaScoreScoring;//and text and such
        score["TEST_PRIMARY_RESULT"] = testThetaSemScoring;
        score["TEST_PRIMARY_RESULT_CODE"] = testThetaSemScoring;
        return score;
    }

    function mapClaimScores(row, testMetadata, signature) {
        var score = {};
        var subject = row.getField("SUBJECT");
        if(subject !== null){
            if(subject.indexOf("ELA") > -1){
