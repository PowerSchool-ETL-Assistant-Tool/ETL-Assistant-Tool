...
var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "AREADING";
    ...
    */
    ...
    var scoresToMap =
        [
           {TEST: "AREADING", CODE:"Fall", FIELD: "FALL_AREADING_", MAP_FUNCTION: mapScaledScore}
           , {TEST: "AREADING", CODE:"Fall_MS", FIELD: "FALL_MS_AREADING_", MAP_FUNCTION: mapScaledScore}
           , {TEST: "AREADING", CODE:"Winter", FIELD: "WINTER_AREADING_", MAP_FUNCTION: mapScaledScore}
           , {TEST: "AREADING", CODE:"Spring", FIELD: "SPRING_AREADING_", MAP_FUNCTION: mapScaledScore}
           , {TEST: "AREADING", CODE:"Spring_MS", FIELD: "SPRING_MS_AREADING_", MAP_FUNCTION: mapScaledScore}
           , {TEST: "AREADING", CODE:"Winter_FTW", FIELD: "_FALL_TO_WINTER", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Spring_FTS", FIELD: "_FALL_TO_SPRING", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Spring_WTS", FIELD: "_WINTER_TO_SPRING", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Winter_FTFM", FIELD: "_FALL_TO_FEB-MAR", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Winter_WTFM", FIELD: "_WINTER_TO_FEB-MAR", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Winter_FMTS", FIELD: "_FEB-MAR_TO_SPRING", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Winter_FTM", FIELD: "FEB-MAR_AREADING_", MAP_FUNCTION: mapScaledScore}
           , {TEST: "AREADING", CODE:"Fall_FTFMS", FIELD: "_FALL_TO_FALL_MS", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Winter_FMSTW", FIELD: "_FALL_MS_TO_WINTER", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Spring_FTSMS", FIELD: "_FALL_TO_SPRING_MS", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Spring_FMSTSMS", FIELD: "_FALL_MS_TO_SPRING_MS", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Spring_WTSMS", FIELD: "_WINTER_TO_SPRING_MS", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Spring_FMSTS", FIELD: "_FALL_MS_TO_SPRING", MAP_FUNCTION: mapGrowthScore}
           , {TEST: "AREADING", CODE:"Spring_SMSTS", FIELD: "_SPRING_MS_TO_SPRING", MAP_FUNCTION: mapGrowthScore}
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row, period) {
        ...
        var springDate = _util.coalesce(row.getField("SPRING_AREADING_FINAL_DATE"),row.getField("SPRING_MS_AREADING_FINAL_DATE"));
        var winterDate = _util.coalesce(row.getField("WINTER_AREADING_FINAL_DATE"),row.getField("FEB-MAR_AREADING_FINAL_DATE"));
        var fallDate = _util.coalesce(row.getField("FALL_AREADING_FINAL_DATE"),row.getField("FALL_MS_AREADING_FINAL_DATE")) ;
        ...
        if(recentDate !== null && recentDate !== undefined) {
            testDate = AppUtil.getMonthFollowedByDay(recentDate).STANDARD_DATE;
        }
        else {
            testDate = "05/15/"+hierarchy.SCHOOL_YEAR.substring(5,9);
        }

        var birthDateFromFile = row.getField("DOB");
        if(birthDateFromFile !== null && birthDateFromFile !== undefined) {
            birthDateObject = AppUtil.getMonthFollowedByDay(birthDateFromFile);
        }
    ...
    function mapScaledScore(row, testMetadata, period, periodIndex) {
        ...
        var lexile = row.getField(testMetadata.FIELD +"LEXILE");
        var uniqueDate = row.getField(testMetadata.FIELD +"FINAL_DATE");
        var lexileScore = removeLexileAlphaChar(_util.coalesce(row.getField(testMetadata.FIELD + "LEXILE")));
        }
    	...
    	if(lexile) {
    	   score["TEST_READING_LEVEL"] = lexile.replace("BR", "").replace("L", "").trim();
        }
    ...
    function mapGrowthScore(row, testMetadata, period, periodIndex) {
        ...
        var growthDistrict = row.getField("DISTRICT_GROWTH_PERCENTILE_FROM"+testMetadata.FIELD);
        var growthNational = row.getField("NATIONAL_GROWTH_PERCENTILE_FROM"+testMetadata.FIELD)
        }
        else
        {
          return null;
       ...
       // We're rejecting the row if test admin date is not present in the row.
        if(period === "Fall") {
            if(!_util.coalesce(row.getField("FALL_AREADING_FINAL_DATE"),row.getField("FALL_MS_AREADING_FINAL_DATE"))) {
                return null;
            }
        } else if(period === "Winter") {
            if(!_util.coalesce(row.getField("WINTER_AREADING_FINAL_DATE"),row.getField("FEB-MAR_AREADING_FINAL_DATE"))) {
                return null;
            }
        } else if(period === "Spring") {
            if(!_util.coalesce(row.getField("SPRING_AREADING_FINAL_DATE"),row.getField("SPRING_MS_AREADING_FINAL_DATE"))) {
                return null;
            }
        }
     ...
     */
     function generateAssessmentAdminKey(signature, hierarchy, srcRecord, period) {
        ...
        // Add the Final date
        if(period === "Fall") {
            if(_util.coalesce(srcRecord.getField("FALL_AREADING_FINAL_DATE"),srcRecord.getField("FALL_AREADING_FINAL_DATE"))) {
                assessmentAdminKey = assessmentAdminKey +  "_" + _util.coalesce(srcRecord.getField("FALL_AREADING_FINAL_DATE"),srcRecord.getField("FALL_AREADING_FINAL_DATE"),"");
            } else {
                // Default date for FALL
                assessmentAdminKey = assessmentAdminKey +  "_" + "09/15/" + "${hierarchy.SCHOOL_YEAR.substring(0,4)}";
            }
        } else if(period === "Winter") {
            if(_util.coalesce(srcRecord.getField("WINTER_AREADING_FINAL_DATE"), srcRecord.getField("FEB-MAR_AREADING_FINAL_DATE"))) {

                assessmentAdminKey = assessmentAdminKey +  "_" + _util.coalesce(srcRecord.getField("WINTER_AREADING_FINAL_DATE"),srcRecord.getField("FEB-MAR_AREADING_FINAL_DATE"),"");
            } else {
                // Default date for WINTER
                assessmentAdminKey = assessmentAdminKey +  "_" + "01/15/" + "${hierarchy.SCHOOL_YEAR.substring(0,4)}";
            }
        } else if(period === "Spring") {
            if(_util.coalesce(srcRecord.getField("SPRING_AREADING_FINAL_DATE"), srcRecord.getField("SPRING_MS_AREADING_FINAL_DATE"))) {
                assessmentAdminKey = assessmentAdminKey +  "_" + _util.coalesce(srcRecord.getField("SPRING_AREADING_FINAL_DATE"),srcRecord.getField("SPRING_MS_AREADING_FINAL_DATE"),"");
            } else {
