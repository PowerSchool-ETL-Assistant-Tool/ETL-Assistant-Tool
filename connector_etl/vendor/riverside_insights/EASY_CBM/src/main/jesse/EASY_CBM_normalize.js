var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "EASY_CBM";
    ...
    var scoresToMap = [
        {TEST: "EASY_CBM", CODE:"RD_LN", FIELD: "LETTER_NAMES", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "EASY_CBM", CODE:"RD_LS", FIELD: "LETTER_SOUNDS", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "EASY_CBM", CODE:"RD_PS", FIELD: "PHONEME_SEGMENTING", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "EASY_CBM", CODE:"RD_PRF", FIELD: "PASSAGE_READING_FLUENCY", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "EASY_CBM", CODE:"RD_VOCAB", FIELD: "VOCABULARY", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "EASY_CBM", CODE:"PROF_RD", FIELD: "PROFICIENT_READING", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "EASY_CBM", CODE:"RD_WRF", FIELD: "WORD_READING_FLUENCY", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "EASY_CBM", CODE:"PROF_MA", FIELD: "PROFICIENT_MATH", MAP_FUNCTION: mapScaledScore}
       ,{TEST: "EASY_CBM", CODE:"BASIC_MA", FIELD: "BASIC_MATH", MAP_FUNCTION: mapScaledScore}
        ...
        normalizedFileFields.forEach(function (field) {
            switch(field) {
                ...
                case "TEST_ADMIN_DATE":
                    record.TEST_ADMIN_DATE = AppUtil.getDateObjByPattern(_util.coalesce(testAdminDate,"05/15/"+hierarchy.SCHOOL_YEAR.substring(5)), "MM/dd/yyyy").STANDARD_DATE;
                    break;
                ...
                case "SCHOOL_VENDOR_ID":
                    record.SCHOOL_VENDOR_ID = _util.coalesce(row.getField("SCHOOL_ID"), row.getField("SCHOOL"), "DISTRICT");
                    break;
    ...
    function mapScaledScore(row, testMetadata) {
		...
		var primary = row.getField("RISK");
		var lexile = row.getField(testMetadata.FIELD + "_LEXILE");
    	...
    	score["TEST_PRIMARY_RESULT"] = primary;
    	score["TEST_PRIMARY_RESULT_CODE"] = primaryDecode[primary];
    ...
    function generateAssessmentAdminKey(signature, hierarchy, row) {
        ...
        var subject = SubjectFromFile(signature, row);
        var period = getTEST_ADMIN_PERIOD(row, hierarchy);
        var date = getAdminDate(row, hierarchy);
        //Add natural key fields
        signature.getFormat().getNaturalKey().forEach(function(key) {
            assessmentAdminKey = assessmentAdminKey + "_" + _util.coalesce(row.getField(key),"");
        });
            assessmentAdminKey = assessmentAdminKey + "_" + date + "_" + period;
        if(subject){
    ...
    function SubjectFromFile(signature, row){
        var sub = null;
        var file = row.getRawField("LINEAGE_FILE").toString().toUpperCase();
        if(file.indexOf('MATH') > -1){
            sub = '_MA'
        } else if(file.indexOf('READING') > -1){
            sub = '_RD'
        }
        return sub;
    }
