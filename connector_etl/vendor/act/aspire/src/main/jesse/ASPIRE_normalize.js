var AppNormalize = (function() {
    ...
    var _assessmentIdentifier = "ACT ASPIRE";
    ...
    function mapNormalizedAdminFields(signature, hierarchy, row) {
        ...
        //added TEST_ADMIN_DATE


        var testDate = _util.coalesceEmptyString(row.getField("ENGLISH TEST ADMINISTRATION DATE"), row.getField("MATH TEST ADMINISTRATION DATE"), row.getField("MATHEMATICS TEST ADMINISTRATION DATE"), row.getField("READING TEST ADMINISTRATION DATE"), row.getField("WRITING TEST ADMINISTRATION DATE"), row.getField("SCIENCE TEST ADMINISTRATION DATE"), row.getField("TEST_ADMIN_DATE"));
        var testDateObject = {
                    RAW_DATE : null
                    , MONTH : null
                    , DAY : null
        ...
        if(testDate !== null && testDate !== "" && testDate !== undefined){
            if (testDate.indexOf("/") >= 2){
                testDateObject = AppUtil.getDateObjByPattern(testDate, "MM/dd/yyyy");
            }
            else if (testDate.indexOf("/") >= 1){
                testDateObject = AppUtil.getDateObjByPattern(testDate, "M/dd/yyyy");
            }
            else if (testDate.indexOf("-") >= 4){
                testDateObject = AppUtil.getDateObjByPattern(testDate, "yyyy-MM-dd");
            }
            else {
                testDateObject = AppUtil.getDateObjByPattern(testDate, "MMddyyyy");
            }
          ...
          var testDateObject = AppUtil.getDateObjByPattern(value, "MMddyyyy");
        }

        var birthDateField = row.getField("STUDENT_BIRTHDATE");
        var birthDateObject = {
            RAW_DATE : null
            , MONTH : null
            , DAY : null
        ...
        if (birthDateField !== null && birthDateField !== "" && birthDateField !== undefined){
            if (birthDateField.indexOf("-") >= 2){
                birthDateObject = AppUtil.getDateObjByPattern(birthDateField, "MM-dd-yyyy");
            }
            else if (birthDateField.indexOf("/") >= 1){
                birthDateObject = AppUtil.getDateObjByPattern(birthDateField, "M/dd/yyyy");
            }
            else {
                birthDateObject = AppUtil.getDateObjByPattern(birthDateField, "MM/dd/yyyy");
            }
    ...
    function mapCompositeScore(row, testMetadata) {
        try {
            var score = {};
            var testNumber = "ASPIRE_" + testMetadata.TEST_NUMBER_SUFFIX;
    ...
    function mapScore(row, test, itr, signature) {
        ...
        var testNumber = "ASPIRE_" + test.TEST_FIELD_ROOT + "_" + itr;
    ...
    function mapRCScore(row, testMetadata) {
        try {
            ...
            var score = {};
            var testNumber = "ASPIRE_";//Subject and other parts must be determined
